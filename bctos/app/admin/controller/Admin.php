...
class Admin extends WebBase
{
    ...
    public function initialize()
    {
        ...
        if (!IS_ROOT && config('app.ADMIN_ALLOW_IP')) {
            // 检查IP地址访问
            if (!in_array(get_client_ip(), explode(',', config('app.ADMIN_ALLOW_IP')))) {
    ...
    {
        $allow = config('app.ALLOW_VISIT');
        $deny = config('app.DENY_VISIT');
    ...
    public function setStatus($Model = CONTROLLER_NAME)
    {
        ...
        switch ($status) {
            case -1:
                return $this->delete($Model, $map, array(
                    'success' => '删除成功',
                    'error' => '删除失败'
                ));
                break;
            case 0:
                return $this->forbid($Model, $map, array(
                    'success' => '禁用成功',
                    'error' => '禁用失败'
                ));
                break;
            case 1:
                return $this->resume($Model, $map, array(
                    'success' => '启用成功',
                    'error' => '启用失败'
                ));
                break;
    ...
    protected function lists_data($model, $where = [], $order = '', $field = true)
    {
        ...
        if (isset($REQUEST['r'])) {
            ...
            $listRows = config('app.LIST_ROWS') > 0 ? config('app.LIST_ROWS') : 20;
        }
