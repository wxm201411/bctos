...
class Follow extends Base
{
    ...
    function init_follow($openid, $pbid = '', $has_subscribe = false, $from_user = [])
    {
        ...
        if ($uid > 0) {
            ...
            if (empty($hasInfo['nickname'])) {
                $user = getWeixinUserInfo($openid, $from_user);
        ...
        // 自动注册
        // $config = getAppConfig('UserCenter', $pbid);
        $user = array(
        ...
        $user2 = getWeixinUserInfo($openid, $from_user);
    ...
    function set_subscribe($user_id, $has_subscribe = 1)
    {
        ...
        $this->where(wp_where($map))->update(['has_subscribe' => $has_subscribe]);
    }
