...
class CategoryTree extends Base
{
	...
	public function getCategoryList($pid = -1, $field = '')
	{
		...
		$data = $this->_model->field($field)->where(wp_where($map))->order('sort ASC')->select();
	...
	public function getCategoryById($id)
	{
		...
		if(!empty($id)) {
			...
			$result = find_data($this->_model->where(wp_where($map))->find());
	...
	{
		$result = $this->_model->where(wp_where('pid='.$pid))->order('sort ASC')->select();
		if($result) {
	...
	public function moveTreeCategory($id, $type)
	{
		...
		// 移动数据
		$pid = $this->_model->where(wp_where($this->_talbe.'_id='.$id))->value('pid');
		$data = $this->_model->field($this->_talbe.'_id, sort')->where(wp_where('pid='.$pid))->order('sort ASC')->select();
		// 存储前后值
		$before = $in = $after = [];
		...
		switch($type) {
			case 'up':
				if(!empty($before)) {
					$this->_model->where(wp_where($this->_talbe.'_id='.$id))->update(['sort'=> $before['sort']]);
					$this->_model->where(wp_where($this->_talbe.'_id='.$before[$this->_talbe.'_id']))->update(['sort'=> $in['sort']]);
					$result = true;
				}
				break;
			case 'down':
				if(!empty($after)) {
					$this->_model->where(wp_where($this->_talbe.'_id='.$id))->update(['sort'=> $after['sort']]);
					$this->_model->where(wp_where($this->_talbe.'_id='.$after[$this->_talbe.'_id']))->update(['sort'=> $in['sort']]);
					$result = true;
				}
				break;
		}
		...
		$result && S('UserCategoryTree', NULL);
		return $result;
	}
	...
	{
		set_time_limit(0);
		$pids = $this->_model->field('DISTINCT pid')->select();
		$pids = getSubByKey($pids, 'pid');
		foreach($pids as $pid) {
			$map['pid'] = $pid;
			$data = $this->_model->where(wp_where($map))->order($this->_talbe.'_id ASC')->select();
			$sort = 1;
			foreach($data as $value) {
				$smap[$this->_talbe.'_id'] = $value[$this->_talbe.'_id'];
				$save['sort'] = $sort;
				$this->_model->where(wp_where($smap))->update($save);
				$sort++;
			}
		}
	}
	 ...
	 */
	public function addTreeCategory($pid, $title, $extra = [])
	{
		// 判断是否有重复的值
		$isExist = $this->_model->where(wp_where("pid={$pid} AND title='{$title}'"))->count();
		if($isExist != 0) {
			return false;
		}
		// 添加分类操作
		...
		// 获取排序值
		$map['pid'] = $pid;
		$maxSort = $this->_model->where(wp_where($map))->order('sort DESC')->value('sort');
		$data['sort'] = $maxSort + 1;
		// 添加额外信息
		if(!empty($extra)) {
			$data = array_merge($data, $extra);
		}
		$result = $this->_model->insertGetId($data);
		// 清除缓存
		$result && $this->remakeTreeCache();
		$result && S('UserCategoryTree', NULL);
		// 删除city.php 文件
	 ...
	 */
	public function upTreeCategory($cid, $title, $extra = [])
	{
		// 判断名称是否重复
		$pid = $this->_model->where(wp_where($this->_talbe.'_id='.$cid))->value('pid');
		$isExist = $this->_model->where(wp_where("pid={$pid} AND title='{$title}'"))->count();
		...
		$result = $this->_model->where(wp_where($map))->update($data);
		// 清除缓存
		$result && $this->remakeTreeCache();
		$result && S('UserCategoryTree', NULL);
		// 删除city.php 文件
	...
	{
		$list = $this->_model->where(wp_where('pid = ' . intval($cid)))->field('area_id')->select();
	...
	public function rmTreeCategory($cid, $_module = null, $_method = null)
	{
		...
		// 判断是否是包含子分类
		$isExist = $this->_model->where(wp_where('pid='.$cid))->count();
		if($isExist != 0) {
			// # 删除该分类下的子分类
			$this->rmRfCate($cid);
		}
		...
		// 删除分类操作
		$map[$this->_talbe.'_id'] = $cid;
		$result = $this->_model->where(wp_where($map))->delete();
		// 执行删除后的关联操作
	...
	public function getCategoryHash($pid = -1)
	{
		$map = [];
		$pid != -1 && $map['pid'] = $pid;
		$data = $this->_model->where(wp_where($map))->order('sort ASC')->getHashList($this->_talbe.'_id', 'title');
	...
	{
		$map[$this->_talbe.'_id'] = $cid;
		$data = find_data($this->_model->where(wp_where($map))->find());
	...
	public function isTitleExist($title)
	{
		...
		$count = $this->_model->where(wp_where($map))->count();
	...
	public function getCategoryAllHash ($pid = -1, $order = 'sort ASC') {
		$map = [];
		$pid != -1 && $map['pid'] = $pid;
		$list = $this->_model->where(wp_where($map))->order($order)->getHashList($this->_talbe.'_id', '*');
		$data = [];
