    ...
    private function parseTemplateFile(string $template): string
    {
        ...
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            if (!is_file($path)) {
                ...
                $theme = get_theme($app);
                if (!empty($theme)) {
                    $theme_path = str_replace('app' . DIRECTORY_SEPARATOR . $app . DIRECTORY_SEPARATOR . 'view', 'public' . DIRECTORY_SEPARATOR . $app . DIRECTORY_SEPARATOR . $theme, $path);
                }
            }
            $view_suffix = ltrim($this->config['view_suffix'], '.');
            if (is_file($theme_path . $template . '.' . $view_suffix)) {
                $template = $theme_path . $template . '.' . $view_suffix;
            } else {
                $template = $path . $template . '.' . $view_suffix;
            }
        }
