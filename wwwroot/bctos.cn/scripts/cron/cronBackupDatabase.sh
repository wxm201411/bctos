...
# 更新备份时间
docker exec panel sh -c "cd /bctos/wwwroot/bctos.cn;php think cron_time ${id} ${size} ${file}.zip '${delFiles}'"
echo "docker exec panel sh -c \"cd /bctos/wwwroot/bctos.cn;php think cron_time ${id} ${size} ${file}.zip '${delFiles}'\""
function timediff() {
    start_time=$1
    end_time=$2
