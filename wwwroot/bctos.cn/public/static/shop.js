...
function addToCart(id, url, buyCount){
	var selfUrl = window.location.href;
	// $.Dialog.loading();
	$.ajax({
		url:url,
		data:{goods_id:id, buyCount:buyCount},
		dataType:'json',
		type:"POST",
		success:function(res){
			if(res.code==1){
				...
				if(res>0){
					if(/collect/.test(selfUrl)) layer.msg('加入购物车成功');
					$('#cartCount').addClass('active');
					$('#cartCount').text(res);
				}else{
				    layer.msg('加入购物车失败，请直接下单购买');
				}
			}
		}
	})
