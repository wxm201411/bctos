...
class explorer extends Controller
{
    ...
    public function pathChmod()
    {
        ...
        foreach ($infoList as $val) {
            $path = _DIR($val['path']);
            if (chmod_path($path, $this->in['mod'])) {
                $success++;
            } else {
                $error++;
            }
    ...
    public function pathRname()
    {
        ...
        if (@bctos_rename($this->path, $rnameTo)) {
    ...
    private function _treeFav()
    {
        ...
        foreach ($favList as $key => $val) {
            ...
            if ($cell['type'] == 'folder' && $cell['ext'] == "") {
                ...
                $cell['exists'] = intval(bctos_file_exists($thePath));
            }
    ...
    private function _groupTree($nodeId)
    ...
    public function pathDelete()
    {
        ...
        if (!path_writeable($userRecycle) || isset($this->in['shiftDelete'])) {//回收站不可写则直接删除；传入直接删除参数
            $removeToRecycle = '0';
        }
        $success = 0;
        $error = 0;
    ...
    public function zip($zipPath = '', $namePre = "", $checkSpaceChange = true)
    {
        ...
        if ($zipPath == '') {
            if (bctos_file_exists($zipname)) {
                $info = LNG('zip_success') . LNG('size') . ":" . size_format(bctos_filesize($zipname));
    ...
    public function image()
    {
        ...
        if (@bctos_filesize($this->path) <= 1024 * 50 ||
        ...
        if (!bctos_file_exists($imageThumb)) {//如果拼装成的url不存在则没有生成过
        ...
        if (!bctos_file_exists($imageThumb) ||
            bctos_filesize($imageThumb) < 100) {//缩略图生成失败则使用原图
            $imageThumb = $this->path;
        }
        ...
        if ($this->in['type'] == 'percent') {//获取下载进度
            if (isset($_SESSION[$uuid])) {
                ...
                $result = array(
                    ...
                    'name' => $info['name'],
                    'size' => (int)@bctos_filesize(iconv_system($info['path'])),
                    'time' => mtime()
                );
                show_json($result);
            } else {
    ...
    private function _pathShare(&$list)
    {
        ...
        foreach ($shareList as $key => $value) {
            ...
            $value['isReadable'] = 1;
            $value['isWriteable'] = 1;
            $value['exists'] = intval(bctos_file_exists($thePath));
    ...
    private function _pathFav(&$list)
    {
        ...
        foreach ($favList as $key => $val) {
            ...
            $cell = array(
                ...
                'isReadable' => 1,
                'isWriteable' => 1,
                'exists' => intval(bctos_file_exists($thePath)),
                'metaInfo' => 'tree-fav',
