...
class user extends Controller
{
    ...
    public function loginCheck()
    {
        ...
        if (isset($_SESSION['kodLogin']) && $_SESSION['kodLogin'] === true && $this->user) {
            ...
            if ($this->config['settingSystem']['autoLogin'] != '1') {
                ...
                if (!bctos_file_exists(USER_SYSTEM . 'install.lock')) {
    ...
    public function login($msg = '')
    {
        ...
        $url = $http . $_SERVER['HTTP_HOST'] . "/index.php?s=/admin/site/files";
        header('location:' . $url);
        exit;
        if (is_wap()) {
    ...
    public function loginFirst()
    {
        if (!bctos_file_exists(USER_SYSTEM . 'install.lock')) {
            ...
            if (!$user['createTime'] ||
                !$user['path'] ||
                !bctos_file_exists(USER_PATH . $user['path'])
            ) {
