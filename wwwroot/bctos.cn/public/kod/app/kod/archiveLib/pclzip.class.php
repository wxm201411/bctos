...
// --------------------------------------------------------------------------------
  ...
  function privFileDescrExpand(&$p_filedescr_list, &$p_options)
  {
    ...
    for ($i=0; $i<sizeof($p_filedescr_list); $i++) {
      ...
      if (bctos_file_exists($v_descr['filename'])) {
  ...
  function privAdd($p_filedescr_list, &$p_result_list, &$p_options)
  {
    ...
    if ((!is_file($this->zipname)) || (bctos_filesize($this->zipname) == 0))
    {
  ...
  function privAddFileList($p_filedescr_list, &$p_result_list, &$p_options)
  {
    ...
    for ($j=0; ($j<sizeof($p_filedescr_list)) && ($v_result==1); $j++) {
      ...
      if (   ($p_filedescr_list[$j]['type'] != 'virtual_file')
          && (!bctos_file_exists($p_filedescr_list[$j]['filename']))) {
...
// TBC : Removed    $p_header['stored_filename'] = $v_stored_filename;
    ...
    if ($p_filedescr['type']=='file') {
      ...
      $p_header['size'] = bctos_filesize($p_filename);
    }
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_ADD].'(PCLZIP_CB_POST_ADD, $v_local_header);');
  ...
  function privAddFileUsingTempFile($p_filedescr, &$p_header, &$p_options)
  {
    ...
    if (bctos_filesize($v_gzip_temp_name) < 18) {
    ...
    @fseek($v_file_compressed, bctos_filesize($v_gzip_temp_name)-8);
    ...
    $p_header['compressed_size'] = bctos_filesize($v_gzip_temp_name)-18;
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_PRE_EXTRACT].'(PCLZIP_CB_PRE_EXTRACT, $v_local_header);');
    ...
    if (bctos_file_exists($p_entry['filename']))
    {
...
//      eval('$v_result = '.$p_options[PCLZIP_CB_POST_EXTRACT].'(PCLZIP_CB_POST_EXTRACT, $v_local_header);');
  ...
  function readZip64ExtraData(&$p_header){
    $size   = bctos_filesize($this->zipname);
  ...
  function privReadEndCentralDir(&$p_central_dir)
  {
    ...
    $v_size = bctos_filesize($this->zipname);
  ...
  function privDuplicate($p_archive_filename)
  {
    ...
    // ----- Copy the files from the archive to the temporary file
    // TBC : Here I should better append the file and go back to erase the central dir
    $v_size = bctos_filesize($p_archive_filename);
    while ($v_size != 0)
    {
      $v_read_size = ($v_size < PCLZIP_READ_BLOCK_SIZE ? $v_size : PCLZIP_READ_BLOCK_SIZE);
      $v_buffer = fread($v_zip_temp_fd, $v_read_size);
