...
// --------------------------------------------------------------------------------
	...
	function PclTarMerge($p_tarname, $p_tarname_add, $p_mode="", $p_mode_add="")
	{
		...
		if ((!is_file($p_tarname)) ||
				(((($v_size = bctos_filesize($p_tarname)) % 512) != 0) && ($p_mode=="tar")))
		{
			// ----- Error log
			if (!is_file($p_tarname))
				PclErrorLog(-4, "Archive '$p_tarname' does not exist");
			else
				PclErrorLog(-6, "Archive '$p_tarname' has invalid size ".bctos_filesize($p_tarname)."(not a 512 block multiple)");
		...
		if ((!is_file($p_tarname_add)) ||
				(((($v_size_add = bctos_filesize($p_tarname_add)) % 512) != 0) && ($p_mode_add=="tar")))
		{
			// ----- Error log
			if (!is_file($p_tarname_add))
				PclErrorLog(-4, "Archive '$p_tarname_add' does not exist");
			else
				PclErrorLog(-6, "Archive '$p_tarname_add' has invalid size ".bctos_filesize($p_tarname_add)."(not a 512 block multiple)");
...
// --------------------------------------------------------------------------------
	...
	function PclTarHandleAddList($p_tar, $p_list, $p_mode, &$p_list_detail, $p_add_dir, $p_remove_dir)
	{
		...
		for ($j=0; ($j<count($p_list)) && ($v_result==1); $j++)
		{
			...
			// ----- Check the filename
			if (!bctos_file_exists($p_filename))
			{
				// ----- Error log
	...
	function PclTarHandleHeader($p_tar, $p_filename, $p_mode, &$p_header, $p_stored_filename)
	{
		...
		else
		{
			...
			// ----- Get the file size
			clearstatcache();
			$v_size = bctos_filesize($p_filename);
		}
...
// var_dump($p_path); changed by warlee;
		...
		While (!($v_end_of_file = ($p_tar_mode == "tar"?feof($v_tar):gzeof($v_tar))))
		{
			...
			if (($v_extract_file) && (!$v_listing))
			{
				...
				if (bctos_file_exists($v_header[filename]))
				{
				...
				//changed by warlee; add $v_header[typeflag]
				{
					...
					if (bctos_file_exists($v_header[filename]) && bctos_filesize($v_header[filename]) != $v_header[size])
					{
						// ----- Close the archive file
						if ($p_tar_mode == "tar")
							fclose($v_tar);
						...
						// ----- Error log
						PclErrorLog(-7, "Extracted file '$v_header[filename]' does not have the correct file size '".bctos_filesize($v_filename)."' ('$v_header[size]' expected). Archive may be corrupted.");
	...
	function PclTarHandleExtractFile($p_tar, &$v_header, $p_path, $p_remove_path, $p_tar_mode)
	{
		...
		$p_remove_path_size = strlen($p_remove_path);
				...
				if (($v_extract_file) && ($v_header[typeflag]!="5"))
				{
					...
					// ----- Check the file size
					clearstatcache();
					if (bctos_filesize($v_header[filename]) != $v_header[size])
					{
						// ----- Error log
						PclErrorLog(-7, "Extracted file '$v_header[filename]' does not have the correct file size '".bctos_filesize($v_filename)."' ('$v_header[size]' expected). Archive may be corrupted.");
