...
/**
 * 系统函数：                filesize(),bctos_file_exists(),pathinfo(),rname(),unlink(),filemtime(),is_readable(),is_wrieteable();
...
{
    return bctos_filesize($path);
}

//文件是否存在，区分文件大小写
function file_exists_case($fileName)
{
    if (bctos_file_exists($fileName) === false) {
        return false;
    }
    $status = false;
 ...
 */
function file_info($path, $time = '', $size = 0)
{
    $info = array(
        ...
        'ctime' => $time ? $time : @filectime($path), //创建时间
        'mtime' => $time ? $time : @filemtime($path), //最后修改时间
        'isReadable' => path_readable($path),
        'isWriteable' => path_writeable($path),
        'size' => $size ? $size : get_filesize($path)
    );
    return $info;
}
 ...
 * 获取文件夹细信息
 */
function folder_info($path, $time = '')
{
    $info = array(
        'name' => iconv_app(get_path_this($path)),
...
{
    if (bctos_is_dir($path) && $same_file_type != 'folder_rename') {//文件夹则忽略
        return $path;
    }
    //重名处理
    if (bctos_file_exists($path)) {
...
{
    if (!bctos_file_exists($path)) return false;
...
{
    $res = ssh_execute("cd $dir; find ./ -type d | wc -l;find ./ -type f | wc -l;du -s");
    $arr = wp_explode(trim($res['msg']));
    $pathinfo['fileCount'] = $arr[1];
    $pathinfo['folderCount'] = $arr[0];
    $pathinfo['size'] = $arr[2];
    return $pathinfo;
}
...
function path_list($dir, $listFile = true, $checkChildren = false)
{
    ...
    foreach ($arr as $fileStr) {
        list($type, $size, $time, $file) = explode("|||", $fileStr);
        if ($file == '.' || $file == '..' || $file == ".svn") continue;
        $type = substr($type, 0, 1);
        $fullpath = $dir . $file;
        if ($type != '-') {
            $info = folder_info($fullpath, $time);
            if ($checkChildren) {
                $info['isParent'] = path_haschildren($fullpath, $listFile);
            }
            $folderList[] = $info;
        } else if ($listFile) {//是否列出文件
            $info = file_info($fullpath, $time, $size);
...
function path_list_bar($dir, $listFile = true, $checkChildren = false)
{
    $dir = rtrim($dir, '/') . '/';
...
function path_haschildren($dir, $checkFile = false)
{
    $dir = rtrim($dir, '/') . '/';
    if ($checkFile) {
        $command = "find $dir -type d -or -type f | wc -l";
    } else {
        $command = "find $dir -type d | wc -l";
    }
    $res = ssh_execute($command);
    if ($res['code'] == 0 && intval($res['msg']) > 0) return true;
    return false;
}

function path_haschildren_bar($dir, $checkFile = false)
{
    $dir = rtrim($dir, '/') . '/';
...
function del_file($fullpath)
{
    if ($fullpath == '/' || strpos($fullpath, '/../') !== false) {
        return false;//禁止危险的删除动作
    }
    $res = ssh_execute("rm -f $fullpath");
    return $res['code'] == 0;
}

function del_file_bar($fullpath)
{
...
function del_dir($dir)
{
    ...
    $res = ssh_execute("rm -rf $dir");
    return $res['code'] == 0;
}

function del_dir_bar($dir)
{
    if (!bctos_file_exists($dir) || !bctos_is_dir($dir)) return true;
    if (!$dh = opendir($dir)) return false;
    @set_time_limit(0);
    while (($file = readdir($dh)) !== false) {
        if ($file == '.' || $file == '..') continue;
        $fullpath = $dir . '/' . $file;
        if (!bctos_is_dir($fullpath)) {
            if (!unlink($fullpath)) { // 删除不了，尝试修改文件权限
                chmod($fullpath, 0777);
                if (!unlink($fullpath)) {
                    return false;
...
function copy_dir($source, $dest)
{
    ...
    $res = ssh_execute("cp -r $source $dest");
    return $res['code'] == 0;
}

function copy_dir_bar($source, $dest)
{
    if (!$dest) return false;
...
{
    $res = ssh_execute("mv -f $source $dest");
    return $res['code'] == 0;
}

function move_file_bar($source, $dest, $repeat_add, $repeat_type)
{
    if ($dest[strlen($dest) - 1] == '/') {
        $dest = $dest . "/" . basename($source);
    }
...
function move_path_bar($source, $dest, $repeat_add = '', $repeat_type = 'replace')
{
    if (!$dest || !bctos_file_exists($source)) return false;
    if (is_dir($source)) {
        //防止父文件夹拷贝到子文件夹，无限递归
 ...
 * @return bool
 */
function mk_dir($dir, $mode = 777)
{
    if (!$dir) return false;
    strlen($mode) == 4 && $mode = substr($mode, 1);
    ssh_execute("[[ -d $dir ]] || mkdir -p $dir; chown -R 82:82 $dir; chmod -R $mode $dir");
    return true;
}

function mk_dir_bar($dir, $mode = 0777)
{
    if (!$dir) return false;
...
function recursion_dir($path, &$dir, &$file, $deepest = -1, $deep = 0)
{
    ...
    $deep += 1;
    if ($deepest == -1) {
        $command = "find $path -type d -or -type f | awk 'NR>1{print $1\"|||\" system(\"[ -d  \"$1\" ]\")}'";
    } else {
        $command = "find $path -type d -or -type f -maxdepth $deep | awk 'NR>1{print $1\"|||\" system(\"[ -d  \"$1\" ]\")}'";
    }
    $res = ssh_execute($command);
    $arr = wp_explode($res['msg']);
    foreach ($arr as $vo) {
...
function recursion_dir_bar($path, &$dir, &$file, $deepest = -1, $deep = 0)
{
    $path = rtrim($path, '/') . '/';
    if (!is_array($file)) $file = array();
    if (!is_array($dir)) $dir = array();
...
{
    return bctos_file_get_contents($file);
}

function file_read_safe_bar($file, $timeout = 3)
{
    if (!$file || !bctos_file_exists($file)) return false;
    $fp = @fopen($file, 'r');
    if (!$fp) return false;
    ...
    if ($locked && bctos_filesize($file) >= 0) {
        $result = @fread($fp, bctos_filesize($file));
        flock($fp, LOCK_UN);
        fclose($fp);
        if (bctos_filesize($file) == 0) {
            return '';
        }
        return $result;
    } else {
...
{
    return bctos_file_put_contents($file, $buffer);
}

function file_wirte_safe_bar($file, $buffer, $timeout = 3)
{
    clearstatcache();
    if (strlen($file) == 0 || !$file || !bctos_file_exists($file)) return false;
    $fp = fopen($file, 'r+');
    $startTime = microtime(true);
    do {
    ...
    if ($locked) {
        ...
        if (!$result || !bctos_file_exists($tempFile)) {
            flock($fp, LOCK_UN);
            fclose($fp);
            return false;
        }
...
function file_search($path, $search, $is_case)
{
    ...
    //文本文件 超过40M不再搜索
    if (@bctos_filesize($path) >= 1024 * 1024 * 40) {
        return false;
    }
    $content = bctos_file_get_contents($path);
 ...
 * @return :string
 */
function chmod_path($path, $mod = '777')
{
    //dump(debug_backtrace());
//    dump($mod);
    if (strlen($mod) == 4) $mod = substr($mod, 1);
//    dump($mod);
//    dump($path);
    //exit;
    $res = ssh_execute("chmod -R $mod $path");
    return $res['code'] == 0;
}

function chmod_path_bar($path, $mod)
{
...
//    header("X-OutFileName: " . $filenameOutput);
    ...
    //输出文件
    $cur = $start;
    echo bctos_file_get_contents($file);
    flush();
//    fseek($fp, $start, 0);
...
function file_download_this($from, $fileName, $headerSize = 0)
{
    ...
    if ($fp = @fopen($from, "rb")) {
        ...
        while (!feof($fp)) {
            if (!bctos_file_exists($fileTemp)) {//删除目标文件；则终止下载
                fclose($downloadFp);
                return false;
            }
            //对于部分fp不结束的通过文件大小判断
...
{
    $res = ssh_execute("[ -d $file ] && ls -l $(dirname $file) | grep $(basename $file) | awk '{print $1}'  || ls -l $file | awk '{print $1}'");

    $theMode = '0' . powerStrToNumber($res['msg']) . powerStrToNumber($res['msg'], 4) . powerStrToNumber($res['msg'], 7);
    //-rwxr-xr-x
    $Mode = substr($res['msg'], 0, 4) . ' ' . substr($res['msg'], 4, 3) . ' ' . substr($res['msg'], 7);

    //drwx r-x r-x(0755)
    return $Mode . '(' . $theMode . ')';
}

function powerStrToNumber($str, $start = 1)
{
    $role = ['r' => 4, 'w' => 2, 'x' => 1, '-' => 0];
    $res = 0;
...
function upload_chunk($uploadFile, $tempPath, $savePath)
{
    ...
    if ($check_md5 !== false) {
        ...
        if (bctos_file_exists($chunk_file) && file_hash_simple($chunk_file) == $check_md5) {
            $arr = array();
            for ($index = 0; $index < $chunks; $index++) {
                if (bctos_file_exists($chunk_file_pre . $index)) {
    ...
    if (kod_move_uploaded_file($uploadFile, $tempFilePre . $chunk)) {
        ...
        if (bctos_file_exists($existMaxFile)) {
        ...
        for ($index = $fromIndex; $index < $chunks; $index++) {
            if (!bctos_file_exists($tempFilePre . $index)) {
