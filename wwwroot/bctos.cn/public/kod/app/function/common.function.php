...
function show_tips($message, $url = '', $time = 3, $title = '', $exit = true)
{
    ...
    if (bctos_file_exists(TEMPLATE . 'common/showTips.html')) {
...
function debug_out()
{
    call_user_func('pr', func_get_args());
}
...
function ssh_execute($command, $show_err = false)
{
    ...
    $command = str_replace('/myos/', '/', $command);
    ...
    if (!empty($result_err)) {
        if (strpos($result_err, '[notice]') === false && strpos($result_err, '[Warning]') === false) {
            return ['code' => 1, 'msg' => trim($result_err)];
        } else {
            return ['code' => 0, 'msg' => trim($output)];
        }
    }
    return ['code' => 0, 'msg' => trim($output)];
}

function bctos_shell_exec($cmd)
{
    $res = ssh_execute($cmd);
    return $res['msg'];
}

function bctos_exec($cmd, &$output = array(), &$return_var = 0)
{
    $res = ssh_execute($cmd);
    $return_var = $res['code'];
...
function bctos_file_exists($file)
{
    $res = ssh_execute("[[ -f $file || -d $file ]] && echo 'yes' || echo 'no'");
    return trim($res['msg']) == 'yes';
}

function bctos_is_file($file)
{
    $res = ssh_execute("[[ -f $file ]] && echo 'yes' || echo 'no'");
    return trim($res['msg']) == 'yes';
}

function bctos_is_dir($dir)
{
    $res = ssh_execute("[[ -d $dir ]] && echo 'yes' || echo 'no'");
    return trim($res['msg']) == 'yes';
}

function bctos_filesize($file)
{
    $res = ssh_execute("ls -l $file | awk '{print $5}'");
    return intval(trim($res['msg']));
}

function bctos_file_get_contents($file)
{
    $res = ssh_execute("cat $file");
    return trim($res['msg']);
}

function bctos_file_put_contents($filename, $data)
{
    $file = '/myos/tmp/file_put_contents_' . time() . rand(10, 99) . 'txt';
    $res = file_put_contents($file, $data);

    ssh_execute("mv -f $file $filename");

    return $res;
}

function bctos_rename($oldname, $newname)
{
    $res = ssh_execute("mv -f $oldname $newname");
    return $res['code'] == 0;
}
