...
function init_common()
{
    ...
    if (!defined('SESSION_PATH_DEFAULT')) {
        //检查session是否存在
        if (!bctos_file_exists(KOD_SESSION) ||
            !bctos_file_exists(KOD_SESSION . 'index.html')) {
            mk_dir(KOD_SESSION);
            touch(KOD_SESSION . 'index.html');
            if (!bctos_file_exists(KOD_SESSION . 'index.html')) {
                show_tips($errorTips);
            }
        }
...
function init_setting()
{
    ...
    if (!bctos_file_exists($settingFile)) {
    ...
    if (!bctos_file_exists($roleGroupFile)) {
    ...
    if (isset($GLOBALS['L']) && is_array($GLOBALS['L'])) {
        I18n::set($GLOBALS['L']);
    }
    I18n::set(array(
...
function hash_path($path, $addExt = false)
{
    ...
    if (bctos_file_exists($path)) {
        ...
        if (bctos_filesize($path) < 50 * 1024 * 1024) {
