...
abstract class InfoBase
{
    ...
    public function __construct()
    {
        ...
        $this->addon_path = SITE_PATH.'/app/'. $this->getName() . '/';
        $TMPL_PARSE_STRING = config('template.TMPL_PARSE_STRING');
        $TMPL_PARSE_STRING['__ADDONROOT__'] = __ROOT__ . '/' . $this->getName();
        config(['TMPL_PARSE_STRING'=>$TMPL_PARSE_STRING], 'template');
    ...
    final protected function fetch($templateFile = CONTROLLER_NAME)
    {
        if (! is_file($templateFile)) {
            ...
            if (! is_file($templateFile)) {
                throw new \think\Exception("模板不存在:$templateFile");
            }
        }
