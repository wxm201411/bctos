...
class AuthRule extends Base
{
    ...
    public function getList()
    {
        ...
        // 从缓存获取不到时从数据库里获取规则
        if (empty($auth_rule)) {
            $list = find_data( $this->field('id,name,mod')->select() );
            foreach ($list as $vo) {
    ...
    {
        $list = find_data( $this->where(wp_where($map))->select() );
        foreach ($list as $v) {
            $lists[] = $v;
        }
        return $lists;
    ...
    public function updateRule($rid, $flag, $id = null, $mod = null)
    {
        ...
        // 已设置的权限节点
        $rule_ids = M('auth_group')->where(wp_where($map))->value('rule');
        if (empty($rule_ids)) {
            $rule_ids = [];
        } else {
        ...
        return M('auth_group')->where(wp_where($map))->update(array(
            'rule' => $data
        ));
    }
