...
// 	    		M('template_messages')->where('id',$id)->update(['send_count'=>$count]);
	    		return $this->success ( '添加' . $model ['title'] . '成功！', U ( 'send_template_message?model=' . $model ['name']  ) );
	    	}else{
	    		return $this->error('群发失败');
	    	}
...
// 	        $map ['manager_id'] = $this->mid;
	        $map ['is_del'] = 0;
	        $group_list = find_data( M ( 'auth_group' )->where ( $map )->select() );
