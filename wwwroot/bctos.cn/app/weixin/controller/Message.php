...
class Message extends WebBase
{
    ...
    public function add()
    {
        ...
        if (request()->isPost()) {
            $data = I('post.');
            /*
             * if (! config( 'app.SEND_GROUP_MSG' )) {
             * return $this->error ( '抱歉，您的公众号没有群发消息的权限' );
             * }
             */
            ...
            if ($id) {
                ...
                return $this->success('添加' . $model['title'] . '成功！', U('add', $this->get_param));
            } else {
            ...
            ! config('app.SEND_GROUP_MSG') && $this->assign('page_tips', '温馨提示：目前微信仅开放认证公众号的群发消息权限，未认证公众号无法使用此功能');
            $this->assign('page_tips', '注意：1、对于认证订阅号，群发接口每天可成功调用1次，此次群发可选择发送给全部用户或某个分组；<br/>
2、对于认证服务号虽然使用高级群发接口的每日调用限制为100次，但是用户每月只能接收4条，无论在公众平台网站上，还是使用接口群发，用户每月只能接收4条群发消息，多于4条的群发将对该用户发送失败；');
            
            $map['pbid'] = get_pbid();
//             $map['manager_id'] = $this->mid;
            $map['is_del'] = 0;
            $group_list = find_data( M('auth_group')->where(wp_where($map))->select() );
    ...
    public function preview()
    {
        ...
        if (empty($openids)) {
            return $this->error('预览OpenID不能为空');
        }
        ...
        if ($count == 0) {
            ...
            return $this->error('有 ' . $count . '条信息发送失败');
        }
    }
    ...
    public function _send_by_group($group_id)
    {
        ...
        $res = $this->postData($url, $paramStr);
    ...
    public function _send_by_openid($openids)
    {
        ...
        if (empty($openids)) {
            return $this->error('要发送的OpenID值不能为空');
        }
        if (count($openids) < 2) {
            return $this->error('OpenID至少需要2个或者2个以上');
        }
    ...
    public function _sucai_media_info()
    {
        ...
        if ($type == 'text') {
            ...
            if (isset($result['errcode']) && $result['errcode'] != 0) {
                return $this->error(error_msg($result));
            }
    ...
    {
        $this->assign('page_tips', '温馨提示<br/>客服群发接口是指：管理者可以给 在48小时内主动发消息给公众号的用户群发消息 ，发送次数没有限制；如果没有成功接收到消息的用户，则在他主动发消息给公众号时，再重新发给该用户。');
        
        // $this->assign ( 'page_tips', '当用户发消息给认证公众号时，管理员可以在48小时内给用户回复信息' );
        if (IS_POST) {
            ...
            if ($sendType == 1 && $sendOpenid == '') {
                return $this->error('指定的Openid值不能为空');
            }
            ...
            if ($count == 0) {
                ...
                return $this->error('有 ' . $count . ' 条消息 发送失败！');
            }
        } else {
            $map['pbid'] = get_pbid();
//             $map['manager_id'] = $this->mid;
    ...
    public function sendOldMessage()
    {
        ...
        $messageData = find_data( M('custom_sendall')->where(wp_where($map))->select() );
        $count = 0;
    ...
    public function _get_user_openid($sendType = 0, $groupId = 0, $openidStr = '')
    {
        ...
        if ($sendType == 0 && $groupId == 0) {
            ...
            $groupData = find_data( M('auth_group_access')->where(wp_where($map1))->select() );
            ...
            foreach ($openids as $op) {
                ...
                if ($uid) {
                    ...
                    return $this->error('Openid为: ' . $op . ' 的用户不存在');
                }
            }
            return $data;
        }
...
//         $map['manager_id'] = $this->mid;
    ...
    private function makeContent($v)
    {
        ...
        if ($type == 'text') {
            ...
            $appMsgData = find_data( M('material_news')->where('group_id', $group_id)->select() );
            if (empty($appMsgData)) {
                $v['Content'] = '图文已删除';
            } else {
...
//         $this->assign('page_tips', '当用户发消息给认证公众号时，管理员可以在48小时内给用户回复信息');
    ...
    public function set_status()
    {
        ...
        $res = M('weixin_message')->where(wp_where($map))->update([$field=>$val]);
        echo $res;
    }
