...
class Wap extends WapBase
{
    ...
    function bind()
    {
        if ((defined('IN_WEIXIN') && IN_WEIXIN) || isset($_GET['is_stree']) || !config('app.USER_OAUTH')) {
            return false;
        }
...
//             $suburl = 'https://api.weixin.qq.com/cgi-bin/user/info?access_token=' . get_access_token() . '&openid=' . $content['openid'] . '&lang=zh_CN';
    ...
    function bind_mobile()
    {
        if (IS_POST) {
            ...
            if (!$user) {
                ...
                $dao->where(wp_where($map))->update(['status'=>0]);
    ...
    function bind_info()
    {
        ...
        if (IS_POST) {
            ...
            if ($res) {
                ...
                return $this->error(D('common/User')->getError());
            }
        } else {
    ...
    function userCenter()
    {
        ...
        // 插件扩展
        $dirs = array_map('basename', glob(SITE_PATH.'/app/*', GLOB_ONLYDIR));
        if ($dirs === false || !file_exists(SITE_PATH.'/app/')) {
            return $this->error('插件目录不可读或者不存在');
        }

        $arr = [];
        $_REQUEST['doNotInit'] = 1;
        foreach ($dirs as $d) {
            require_once SITE_PATH.'/app/'. $d . '/model/WeixinAddon.php';
    ...
    function credit()
    {
        ...
        return parent::commonLists($model, 'common@base/lists');
    }
    ...
    function storeCenter()
    {
        ...
        $list = find_data( M('coupon')->where(wp_where($map))->select() );
