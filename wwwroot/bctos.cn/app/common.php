...
function D($name = '', $layer = 'model')
{
    ...
    $class = check_model($name, $module_name);
    if ($class === false) {
        throw new \think\Exception($module_name . '/' . $name . '模型不存在', 10006);
    }
    $model = new $class;
...
function check_model($name, $module_name = '')
{
    ...
    // 兼容没有对应的model文件的写法
    $path = SITE_PATH . '/app/' . $module_name . '/model/' . $name . '.php';
    $common = SITE_PATH . '/app/common/model/' . $name . '.php';

    if (file_exists($path)) {
        return '\\app\\' . $module_name . '\\model\\' . $name;;
    } elseif (file_exists($common)) {
        return '\\app\\common\\model\\' . $name;;
    } else {
        return false;
    }
}
...
function U($url = '', $vars = '', $suffix = true, $domain = true)
{
    ...
    $varsArr = [];
    if (is_string($vars)) {
        parse_str($vars, $varsArr);
    } else {
        $varsArr = $vars;
    }
    //应用修改为小写格式，防止在liunx下报错
    $url_parts = explode('/', $url);
    if (count($url_parts) > 2) {
        $app = array_shift($url_parts);
        $url = parse_name_lower($app) . '/' . implode('/', $url_parts);
    }
    $url = url($url, $varsArr, $suffix, $domain);
...
function W($name, $data = [])
{
    ...
    $count = count($nameArr);
    $app = parse_name($count >= 3 ? $nameArr[0] : MODULE_NAME);
    $contoller = parse_name(($count == 2 ? $nameArr[0] : $nameArr[1]), 1);
    $action = $nameArr[$count - 1];

    // 兼容没有对应的model文件的写法
    $path = SITE_PATH . '/app/' . $app . '/widget/' . $contoller . '.php';

    if (file_exists($path)) {
        $class = '\\app\\' . $app . '\\widget\\' . $contoller;;
    } else {
...
{
    //取值需要把null返回值换成false,解决TP6的BUG
    if ($value === '' && false === strpos($name, '?')) {
        if (false === S('?' . $name)) {
            return false;
        }
    }
...
function is_admin($uid = null)
{
    ...
    // 是否为运营管理员
    $res = M('auth_group_access')->alias('a')
        ->join(DB_PREFIX . 'auth_group b', 'a.group_id=b.id')
        ->where('uid', $uid)
        ->column('b.type');
...
{
    $uid = is_null($uid) ? is_login() : $uid;
    return $uid && (intval($uid) == config('app.user_administrator'));
}
...
function public_interface($auth, $mod = 'weixin_public', $pbid = null)
{
    ...
    $rule = S('auth_group_rule_type' . $info['type']);

    if ($rule === false) {
        $rule = M('auth_group')->where('wtype', $info['type'])->value('rule');
        $rule = explode(',', $rule);
        S('auth_group_rule_type' . $info['type'], $rule);
    }
...
function getUserBaseInfo($data)
{
    ...
    $data['remark'] = isset($user['remark']) ? $user['remark'] : '';
    return $data;
}

function getUserInfoByUids($uids)
{
    is_array($uids) or $uids = explode(',', $uids);
    $res = [];
    foreach ($uids as $uid) {
        $res [$uid] = getUserInfo($uid);
    }
...
function execute_action($rules = false, $action_id = null, $user_id = null)
{
    ...
    foreach ($rules as $rule) {
        ...
        $field = $rule['field'];
        $res = $Model->where(wp_where($rule['condition']))->update([$field => $rule['rule']]);
 ...
 * 获取属性信息并缓存
 */
function get_model_attribute($model, $obj = false)
{
    $obj == false && $obj = D('common/Models')->getFileInfo($model);
    if ($obj == false) return [];


    return extra_array($obj->fields);
}

function extra_array($fields = [])
{
    foreach ($fields as &$f) {
        if ($f['type'] == 'dynamic_select' || $f['type'] == 'dynamic_checkbox') {
            list($options, $first_option) = dynamic_data($f);
            if ($f['type'] == 'dynamic_select') {
                $f['extra_array'][] = $first_option;
                foreach ($options as $k => $v) {
                    $f['extra_array'][$k] = $v;
                }
            } else {
                $f['extra_array'] = $options;
            }
        } else if (isset($f['extra'])) {
            $f['extra_array'] = parse_field_attr($f['extra']);
        }
        if (isset($f['extra_array'])) {
            foreach ($f['extra_array'] as $k => $e) {
                $f['extra_array_id'][] = $k;
                $f['extra_array_title'][] = $e;
            }
        }
    }
    return $fields;
...
function dynamic_data($data)
{
    $key = 'dynamic_' . $data ['name'] . '_' . get_wpid();
    $res = S($key);

    if ($res === false || true) {
        $manager_id = $GLOBALS ['mid'];
        $wpid = get_wpid();
        ...
        // dump($arr);
        //$arr ['wpid'] = get_wpid ();
        $list = find_data(M($table)->where(wp_where($arr))->field($value_field . ',' . $title_field)->order($order)->select());
...
function get_cover_url_mult($cover_ids, $width = '', $height = '')
{
    $res = [];
    if (empty($cover_ids)) {
        return $res;
    }
    $cover_ids = array_filter(explode(',', $cover_ids));
    foreach ($cover_ids as $cover_id) {
        $res[] = get_cover_url($cover_id, $width, $height);
    }
    return $res;
}

function get_cover_url($cover_path, $width = '', $height = '')
{
    if ($width || $height) {
        $path = mk_rule_image($cover_path, $width, $height);
        return $path;
    } else {
        $url = substr($cover_path, 0, 4) != 'http' ? SITE_URL . $cover_path : $cover_path;
...
function get_file_info($id)
{
    ...
    if (isset($file['url']) && $file['url']) {
        ...
        $url = SITE_URL . '/storage/download/' . $file['savepath'] . $file['savename'];
    }
...
function getOpenidByUid($uid, $pbid = '')
{
    ...
    $map ['uid'] = $uid;
    $map ['pbid'] = $pbid;
    return M('public_follow')->where(wp_where($map))->value('openid');
}
 ...
 * 微信支付和红包使用
 */
function getPaymentOpenid($appId = "", $serect = "", $notLeYao = true, $uid = 0)
{
    ...
    $callback = GetCurUrl();
    if ($uid) {
        $callback .= '&save_uid=' . $uid;
        $callback = setDefaultUrl($callback, $uid);
        addWeixinLog($callback, 'testcallback');
    }
    $param = $appId . ':' . $serect;
    $openid = OAuthWeixin($callback, $param, true, $notLeYao);
...
{
//     return 1;//TODO 调试期间设置为单社区
    $domain = dns_get_record(SITE_DOMAIN, DNS_A);
    if (empty ($domain)) {
        $domain = explode('.', SITE_DOMAIN);
    } else {
        $domain = explode('.', $domain [0] ['host']);
    }
    if (count($domain) == 4 && SITE_DOMAIN != 'localhost') { // 泛域名支持
        $id = domain_decode($domain [1]);
    }
    if (empty($id)) {
        $id = getUserInfo(WPID, 'social_id');
    }
...
function getWeixinUserInfo($openid, $from_user = [])
{
    /*
     * if (! config('app.USER_BASE_INFO')) {
     * return [];
     * }
     */
    if (!empty($from_user))
...
function OAuthWeixin($callback, $pbid = '', $is_return = false, $notLeYao = true)
{
    if ((defined('IN_WEIXIN') && IN_WEIXIN) || isset($_GET['is_stree']) || (!config('app.USER_OAUTH') && $notLeYao)) {
        return false;
    }
...
// 截取内容
function getShort($str, $length = 40, $ext = '', $mbStr = false)
{
...
function add_credit_by_uid($uid, $name, $lock_time = 5, $credit = [], $admin_uid = 0)
{
    add_credit($name, $credit, $lock_time, $admin_uid, $uid);
}
...
{
    require_once SITE_PATH . '/vendor/' . 'download-xlsx.php';
...
function importFormExcel($attach_id, $column, $dateColumn = [])
{
    ...
    $root = config('app.DOWNLOAD_UPLOAD.rootPath');
    $filename = SITE_PATH . '/public/storage/download/' . $file['savepath'] . $file['savename'];
    ...
    require_once SITE_PATH . '/vendor/PHPExcel.php';
    require_once SITE_PATH . '/vendor/PHPExcel/IOFactory.php';
    require_once SITE_PATH . '/vendor/PHPExcel/Reader/Excel5.php';
...
function showNewIcon($time, $day = 3)
{
    ...
    if (NOW_TIME < ($time + 86400 * $day)) {
        $img = '<img src="' . config('template.TMPL_PARSE_STRING.__IMG__') . '/new.png"/>';
    }
    return $img;
}
 ...
 * 获取插件的配置数组
 */
function getAppConfig($name, $pbid = 0)
{
...
{
    return wp_money_format($money / 100);
}
...
{
    if (!config('app.DIV_DOMAIN') || is_numeric($domain) || SITE_DOMAIN == 'localhost' || empty($domain)) {
        return $url;
    }
...
function top_domain()
{
    if (strpos(SITE_DOMAIN, 'bctos.cn') !== false) {
        return '.bctos.cn';
    } else {
        return '.oftenchat.cn';
    }
...
{
    $savePath = SITE_PATH . '/public/storage/picture/' . time_format(NOW_TIME, 'Y-m-d');
    mkdirs($savePath);
    // 获取图片URL
    ...
    if ($res) {
        ...
        $pic_driver = config('app.PICTURE_UPLOAD_DRIVER');
        $files = request()->file();
        $info = $Picture->upload($files, config('app.PICTURE_UPLOAD'), config('app.PICTURE_UPLOAD_DRIVER'), config("app.UPLOAD_{$pic_driver}_CONFIG"), true);
...
{
    $savePath = SITE_PATH . '/public/storage/picture/' . time_format(NOW_TIME, 'Ymd');
    mkdirs($savePath);
    if (empty($picUrl)) {
        // 获取图片URL
...
{
    $savePath = SITE_PATH . '/public/storage/download/' . time_format(NOW_TIME, 'Y-m-d');
    mkdirs($savePath);
    // 获取图片URL
    ...
    if ($res) {
        ...
        $File = D('home/File');
        $file_driver = config('app.DOWNLOAD_UPLOAD_DRIVER');
        $info = $File->upload($_FILES, config('app.DOWNLOAD_UPLOAD'), config('app.DOWNLOAD_UPLOAD_DRIVER'), config("app.UPLOAD_{$file_driver}_CONFIG"), true);
        $cover_id = $info['download']['id'];
        @unlink($picPath);
    }
    return $cover_id;
...
{
    require_once SITE_PATH . "/vendor/emoji.php";
...
function matchImages($content = '', $return_ids = false)
{
    $src = [];
...
function makeConverId($picUrl)
{
    $info = pathinfo($picUrl);
    if (!$info)
        return 0;

    $picPath = str_replace(SITE_URL, SITE_PATH, $picUrl);
    !file_exists($picPath) && file_exists(SITE_PATH . $picPath) && $picPath = SITE_PATH . $picPath;
    $res = true;
    $cover_id = 0;
    $Picture = D('Home/Picture');
    if (!file_exists($picPath)) {
        ...
        $picPath = $savePath . '/' . $info ['basename'];
        $res = file_put_contents($picPath, $content);
    } else {
        $map ['md5'] = md5_file($picPath);
        $cover_id = intval($Picture->where(wp_where($map))->value('id'));
    }
    ...
    if ($res && $cover_id == 0) {
        ...
        $pic_driver = config('app.PICTURE_UPLOAD_DRIVER');
        $info = $Picture->upload($_FILES, config('app.PICTURE_UPLOAD'), config('app.PICTURE_UPLOAD_DRIVER'), config("app.UPLOAD_{$pic_driver}_CONFIG"));
        $cover_id = $info ['download'] ['id'];
    }
    // dump ( $cover_id );
    return $cover_id;
...
{
    $badword_arr = explode(',', config('app.SENSITIVE_WORDS'));
 ...
 * @return bool
 */
function wpload($filename)
{
    static $_importFiles = []; // 已载入的文件列表缓存
...
// 调用接口服务
function X($name, $params = [], $domain = 'model')
{
    return D($name);
}
 ...
 * html标签有格式输出内容
 */
function html_entity_decode_out($content)
{
 ...
 * $is_true false 普通管理员
 */
function is_social_admin($uid, $social_id = 0, $is_true = false)
{
    if (( int )$uid < 1)
        return false;
    if ($uid == 1)
        return true;
    $get_url_social_id = get_social_id();
    empty($social_id) && $social_id = session('social_id') == $get_url_social_id ? session('social_id') : $get_url_social_id;
    ...
    $admin = M('social')->where(wp_where($map))->find();
    if (!empty ($admin)) {
    ...
    // 判断是否为一般社区管理员
    // $social_rule = get_userinfo ( $uid, 'social_rule' );
    // 暂时返回false
    return false;
}
 ...
 * 是否为社区圈子管理员（吧主）
 */
function is_weiba_admin($uid, $social_id, $weiba_id)
{
    // if ($social_id == '' || $social_id < 1) {
    $social_id = get_social_id();
    // }

    $weiba_admin = M('weiba')->where(array(
        'id' => $weiba_id
    ))->value('admin_uid');
    if (in_array($uid, explode(',', $weiba_admin))) { // 吧主
        return true;
    } else { // 管理员
        $is_social_admin = is_social_admin($uid, $social_id);
        if ($is_social_admin) { // 如果是社区管理员直接返回true
            return true;
        }
    }
    return false;
...
function CheckWeibaPermission($weiba_admin, $id, $action, $uid = 0)
{
    ...
    if (!$weiba_admin && $id) {
        ...
        $weiba_admin = M('weiba_follow')->where(wp_where($map))->order('level desc')->field('follower_uid,level')->findAll();
        $weiba_admin = getSubByKey($weiba_admin, 'follower_uid');
    }
    return in_array($uid, $weiba_admin);
}

function checkCityAdmin($cityId, $uid)
{
    !$uid && $uid = $GLOBALS ['mid'];
    // 超级管理员判断
    if (empty ($cityId))
        return false;
    $cityInfo = M('city')->find($cityId);
    $uidArr = array_filter(explode(',', $cityInfo ['manager_uids']));
    foreach ($uidArr as $u) {
        if (intval($u) == $uid) {
            return true;
            break;
...
function saveImageToLocal($url)
{
    ...
    $opts = array(
        'http' => array(
            // 'header'=>"Referer:http://note.youdao.com/",
            'method' => "GET",
            'timeout' => 30, // 超时30秒
    ...
    if ($res) {
        return '/uploads' . $file_path . $file_name;
    } else {
        return false;
    }
}
 ...
 * /storage/Editor/gh_dd85ac50d2dd/2016-08-26/57bfa4a23fba5.png
 * http://localhost/wp6/public/static/icon/colors/1187213.png
 */
function mk_rule_image($imgurl, $w, $h)
{
    if (preg_match('#^/static/icon/#i', $imgurl)) {
        // 内部图标
        $img = $imgurl;
        $imgurl = '.' . $img;
        ...
        $dirname = str_replace('./static', './storage', $dirname);
        if (!is_dir($dirname)) {
            mkdirs($dirname);
        }
        ...
        if (file_exists($dirname_new)) {
            return str_replace('./storage', SITE_URL . '/storage', $dirname_new);
        }
    ...
    if (preg_match('#^/storage/picture/#i', $imgurl) || preg_match('#^/static/icon/#i', $imgurl)) {
        // 内部图片
        $img = $imgurl;
        $imgurl = '.' . $img;
        $filename = basename($imgurl);
    ...
    if (preg_match('#^(http|https)://#i', $imgurl)) {
        ...
        if ($imginfo) {
            // 规格图片存在
            if ($imginfo[0] > $w || $imginfo[1] > $h) {
                $save_filename = './storage/picture/' . $filename;
...
function upload_files($setting = '', $driver = '', $config = '', $type = 'picture', $isTest = false)
{
    ...
    if ($type == 'picture') {
        //图片扩展名验证 ，图片大小不超过20M
        $checkRule['fileExt'] = 'gif,jpg,jpeg,png,bmp';
        $checkRule['filesize'] = 20971520;
    } else {
        $allowExt = input('allow_file_ext', '');
        if ($allowExt != '') {
            $checkRule['fileExt'] = $allowExt;
        }
        $allowSize = input('allow_file_maxsize', '');
        if ($allowSize > 0) {
            $checkRule['filesize'] = intval($allowSize);
        }
    }
    try {
        validate([$key => $checkRule])->check($files);
    ...
    if ($savename) {
        $return['mime'] = $file->getMime();
        $return['name'] = $file->getOriginalName();
        $return['key'] = $key;
        $return['ext'] = $extension;
        $return['savename'] = '/' . str_replace('\\', '/', $savename);
        $return['md5'] = $file->md5();
        $return['sha1'] = $file->sha1();
        $return['code'] = 1;

        $return['size'] = 0;
        $return['rootPath'] = '/' . config('filesystem.disks.public.url');
    } else {
...
function wp_having($map)
{
    $wp_where = wp_where($map);
    //dump($wp_where);
    if (is_array($wp_where)) {
        $having = [];
        foreach ($wp_where as $vo) {
            if (is_array($vo)) {
                $having[] = is_numeric($vo[2]) ? $vo[0] . ' ' . $vo[1] . ' ' . $vo[2] : $vo[0] . ' ' . $vo[1] . " '" . $vo[2] . "'";
            } else {
                $having[] = $vo;
            }
        }
...
function file_log($content, $file_name = "debug", $title = '')
{
    ...
    $logPath = SITE_PATH . "/runtime/" . $file_name . date("Y-m-d") . ".log";

    error_log("\r\n" . date("[Y-m-d H:i:s]") . " : " . $content, 3, $logPath);
}

// 自定义域名加密
function domain_encode($social_id)
{
    $code = 'abcdefghijklmnopqrstuvwxyz';
    $str = ( string )$social_id;
    // 不足3位补全3位
    if ($social_id < 10) {
        $str = '00' . $social_id;
    } elseif ($social_id < 10) {
...
// 自动跳转到二级域名下
function domain_url($url, $domain)
{
    if (SITE_DOMAIN == 'club.jindanke.cn') {
        $url = str_replace('club.jindanke.cn', "social.{$domain}.bctos.cn", $url);
        return $url;
    } else {
        return $url;
    }
 ...
 * url:支付跳url
 * uid:当前用户
 * 支付跳转时，域名切换为club.jindanke.cn
 */
function setDefaultUrl($url, $uid)
{
    $str = SITE_DOMAIN;
    if ($str != 'club.jindanke.cn') {
        $domain = preg_replace('/^social\.|\.bctos.cn$/u', '', $str);
        $key = 'weiba_user_domain_' . $uid;
        S($key, $domain);
        $url = str_replace($str, 'club.jindanke.cn', $url);
        return $url;
    } else {
        return $url;
    }
...
// 截取带html标签的字符串，html标签不占长度
function html_substr($str, $len)
{
    $ar = preg_split('/(<img[^>]*\>)/', $str, -1, PREG_SPLIT_NO_EMPTY | PREG_SPLIT_DELIM_CAPTURE);
    ...
    foreach ($ar as $k => $v) {
        ...
        if (substr($v, 0, 4) != '<img') {
            ...
            if (mb_strlen($v, 'utf-8') >= $len) {
                ...
                $nar [$k] = mb_substr($v, 0, $len, 'utf-8');
                $nar [$k + 1] = '...';
                break;
            } else {
                $i++;
                $nar [$k] = $v;
                $len = $len - mb_strlen($v, 'utf-8');
                if ($i >= 15) {
                    break;
                }
            }
...
// //获取收费配置标准
function get_social_charge()
{
    $charge = config('app.SOCIAL_CHARGE');
    $year = array(
        '免费',
        '一年',
        '两年',
...
function G_WP($tag, $flat = 'show')
{
    G($tag);
    if ($flat != null) {
        isset($_GET['debug']) && dump($tag . ': ' . G($GLOBALS['g_wp'], $tag));
    }
    $GLOBALS['g_wp'] = $tag;
}

function feed_content_parse($str, $getShort = false)
{
    $getShort && $str = getShort($str, 50, '...');
    ...
    foreach ($matches as $k => $m) {
        $m = ltrim($m, '[');
        $m = rtrim($m, ']');
        $icons [$k] = '<img width="30px" height="30px" src="' . SITE_URL . '/static/face/ts/' . $m . '.gif" />';
    }
    return str_replace($matches, $icons, $str);
 ...
 */
function get_cover_urlByWeiba($attachid)
{
...
{
    $maps ['id'] = $map ['weiba_id'] = intval($weiba_id);
    $map ['is_del'] = 0;

    $save ['thread_count'] = M('feed')->where(wp_where($map))->count();
    $save ['post_count'] = M('feed_reply')->where(wp_where($map))->count();
    ...
    $post_count = M('feed')->where(wp_where($map))->count();
    unset($map ['cTime']);
    ...
    $reply_count = M('feed_reply')->where(wp_where($map))->count();

    $save ['today_count'] = $post_count + $reply_count;

    unset($map ['time']);
    $res = D('social/Weiba')->where(wp_where($maps))->update($save);
    // dump($res);
    // dump(D('social/Weiba')->getLastSql());exit;
}

function refreshWeibaCount($weiba_id = 0, $field = array('follow_count', 'thread_count', 'post_count', 'today_count'))
{
    $map ['weiba_id'] = $maps ['id'] = $weiba_id;

    // 会员数
    in_array('follow_count', $field) && $data ['follow_count'] = M('weiba_follow')->where(wp_where($map))->count();
    // 主题数
    $map ['is_del'] = 0;
    in_array('thread_count', $field) && $data ['thread_count'] = M('feed')->where(wp_where($map))->count();
    // 帖子数
    in_array('post_count', $field) && $data ['post_count'] = M('feed_reply')->where(wp_where($map))->count();
    ...
    if (in_array('today_count', $field)) {
        ...
        $data ['today_count'] = M('feed')->where(wp_where($map))->count();

        $map ['cTime'] = array(
            '>',
            $ytime
        );
        $data ['yesterday_count'] = M('feed')->where(wp_where($map))->count();
        ...
        $data ['today_count'] = M('feed_reply')->where(wp_where($map))->count() + $data ['today_count'];

        $map ['ctime'] = array(
            '>',
            $ytime
        );
        $data ['yesterday_count'] = M('feed_reply')->where(wp_where($map))->count() + $data ['yesterday_count'] - $data ['today_count'];

        $data ['today_date'] = date('Ymd');
    }
    $res = M('weiba')->where(wp_where($maps))->update($data);
...
function parsetable($width, $bgcolor, $message)
{
    if (strpos($message, '[/tr]') === FALSE && strpos($message, '[/td]') === FALSE) {
        ...
        return (!defined('IN_MOBILE') ? '<table cellspacing="0" class="t_table" ' . ($width == '' ? NULL : 'style="width:' . $width . '"') . ($bgcolor ? ' bgcolor="' . $bgcolor . '">' : '>') : '<table>') . str_replace('\\"', '"', preg_replace_callback(array(
                "/\[tr(?:=([\(\)\s%,#\w]+))?\]\s*\[td(?:=(\d{1,4}%?))?\]/i",
                "/\[\/td\]\s*\[td(?:=(\d{1,4}%?))?\]/i",
                ...
                function ($r) {
                    return parsetrtd('\\1', '0', '0', '\\2');
                },
                function ($r) {
                    return parsetrtd('td', '0', '0', '\\1');
                },
                function ($r) {
                    return parsetrtd('\\1', '\\2', '\\3', '\\4');
                },
                function ($r) {
                    return parsetrtd('td', '\\1', '\\2', '\\3');
                },
                '</td></tr>'
            ), $message)) . '</table>';
    }
...
function parsemedia($params, $url)
{
    ...
    if (in_array(count($params), array(
        3,
        4
    ))) {
        $type = $params [0];
        $url = htmlspecialchars(str_replace(array(
            '<',
            '>'
        ), '', str_replace('\\"', '\"', $url)));
        switch ($type) {
...
function bbcodeurl($url, $tags)
{
    if (!preg_match("/<.+?>/s", $url)) {
        ...
        return str_replace(array(
            ...
            '',
            ''
        ), str_replace('{url}', addslashes($url), $tags));
    } else {
        return '&nbsp;' . $url;
    }
}
...
function bbcode($message)
{
    ...
    if (strpos($msglower, '[/url]') !== FALSE) {
        $message = preg_replace_callback("/\[url(=((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|thunder|qqdl|synacast){1}:\/\/|www\.|mailto:)?([^\r\n\[\"']+?))?\](.+?)\[\/url\]/is", function ($r) {
            return parseurl('\\1', '\\5', '\\2');
        }, $message);

    }

    if (strpos($msglower, '[/email]') !== FALSE) {
        $message = preg_replace_callback("/\[email(=([a-z0-9\-_.+]+)@([a-z0-9\-_]+[.][a-z0-9\-_.]+))?\](.+?)\[\/email\]/is", function ($r) {
            return parseemail('\\1', '\\4');
        }, $message);
    }

    $nest = 0;
    while (strpos($msglower, '[table') !== FALSE && strpos($msglower, '[/table]') !== FALSE) {
        $message = preg_replace_callback("/\[table(?:=(\d{1,4}%?)(?:,([\(\)%,#\w ]+))?)?\]\s*(.+?)\s*\[\/table\]/is", function ($r) {
            return parsetable('\\1', '\\2', '\\3');
        }, $message);
        if (++$nest > 4)
            break;
    }
    ...
    if (strpos($msglower, '[/media]') !== FALSE) {
        $message = preg_replace_callback("/\[media=([\w,]+)\]\s*([^\[\<\r\n]+?)\s*\[\/media\]/is", $allowmediacode ? function ($r) {
            return parsemedia('\\1', '\\2');
        } : function ($r) {
            return bbcodeurl('\\2', '<a href=\"{url}\" target=\"_blank\">{url}</a>');
        }, $message);
    }
    if (strpos($msglower, '[/audio]') !== FALSE) {
        $message = preg_replace_callback("/\[audio(=1)*\]\s*([^\[\<\r\n]+?)\s*\[\/audio\]/is", $allowmediacode ? function ($r) {
            return parseaudio('\\2', 400);
        } : function ($r) {
            return bbcodeurl('\\2', '<a href=\"{url}\" target=\"_blank\">{url}</a>');
        }, $message);
    }
    if (strpos($msglower, '[/flash]') !== FALSE) {
        $message = preg_replace_callback("/\[flash(=(\d+),(\d+))?\]\s*([^\[\<\r\n]+?)\s*\[\/flash\]/is", $allowmediacode ? function ($r) {
            return parseflash('\\2', '\\3', '\\4');
        } : function ($r) {
            return bbcodeurl('\\4', '<a href=\"{url}\" target=\"_blank\">{url}</a>');
        }, $message);
    }
...
function headimgurl($uid, $url)
{
    // 用户头像
    $md5 = md5($uid);
    $sc = '/' . substr($md5, 0, 2) . '/' . substr($md5, 2, 2) . '/' . substr($md5, 4, 2);
    $path = SITE_PATH . '/uploads/user_face' . $sc . '/big.jpg';
    if (file_exists($path)) {
        $res = SITE_URL . '/uploads/user_face' . $sc . '/big.jpg'; // TODO 后续优化可以切小图
    } else {
        $res = empty ($url) ? SITE_URL . '/static/face/default_head_50.png' : $url;
    }
    return $res;
}
...
function keys_list($table_name)
{
    ...
    // dump($list_keys);
    if ($list_keys === false) {
        $list_keys = find_data(M('cache_keys')->where('table_name', $table_name)->select());
        S('keyscache_' . $table_name, $list_keys);
    }
    return empty($list_keys) ? [] : $list_keys;
}
...
function get_cert_pem($config)
{
    ...
    $path = SITE_PATH . '/public/storage/download';
...
//获取应用信息
function get_app_info($app_name, $field = null)
{
    //TODO 待增加缓存
    $info = M('apps')->where('name', $app_name)->find();
    if ($field !== null) {
...
//获取应用的主题
function get_theme($app_name)
{
    return get_app_info($app_name, 'theme');
}

//在列表中显示树形结构
function treeList($data, $pid = 0, $level = '', $need_a = true)
{
    $tree = [];
    ...
    foreach ($data as $vo) {
        if ($pid == $vo['pid']) {
            if ($need_a) {
                $vo['title'] = '<a style="color:#1E9FFF" href="' . U('lists?id=' . $vo['id']) . '">' . $level . $vo['title'] . '</a>';
            } else {
                $vo['title'] = $level . $vo['title'];
            }
            $tree[] = $vo;
            //查找下级
            $child = treeList($data, $vo['id'], $child_level, $need_a);
            if (!empty($child)) {
...
//从数据模型中的字段定义中获取表单的默认值
function default_form_value($fields)
{
    $data = [];
    foreach ($fields as $f) {
        $data[$f['name']] = $f['value'];
...
function get_model($model = null)
{
    ...
    if (!$model) {
        CONTROLLER_NAME != 'api' && $model = getModelByName(CONTROLLER_NAME);
        if (!$model) {
            $model = getModelByName(MODULE_NAME . '_' . parse_name(CONTROLLER_NAME));
        }
        if (!$model) {
...
function up_chain($func, $param = [], $isQuery = 0, $peerConfig = [])
{
    isset($peerConfig['paramType']) || $peerConfig['paramType'] = 'array'; //通道名
    isset($peerConfig['channelName']) || $peerConfig['channelName'] = 'mychannel'; //通道名
    isset($peerConfig['chaincodeId']) || $peerConfig['chaincodeId'] = 'clockin'; //链码ID
    isset($peerConfig['CAUrl']) || $peerConfig['CAUrl'] = 'ca.org1.example.com'; //CA证书网址
    isset($peerConfig['mspId']) || $peerConfig['mspId'] = 'Org1MSP'; //管理员和用户所属组织
    isset($peerConfig['userName']) || $peerConfig['userName'] = 'xiaowei'; //用户名
    isset($peerConfig['userDepartment']) || $peerConfig['userDepartment'] = 'org1.department1'; //用户部门
    isset($peerConfig['adminName']) || $peerConfig['adminName'] = 'admin'; //管理员账号
    isset($peerConfig['adminPasswd']) || $peerConfig['adminPasswd'] = 'adminpw'; //管理员密码
    isset($peerConfig['ccpPath']) || $peerConfig['ccpPath'] = getenv('GOPATH') . '/src/github.com/hyperledger/fabric-samples/test-network/organizations/peerOrganizations/org1.example.com/connection-org1.json'; //通道名
...
function get_app_config_file($app, $controller = '')
{
    $fields = [];

    $file = SITE_PATH . '/app/' . $app . '/config.php';
    if (file_exists($file)) {
        $re_arr = include $file;
        if (!empty($controller) && isset($re_arr[$controller])) {
            $fields = $re_arr[$controller];
        } else {
            foreach ($re_arr as $test) {
                if (isset($test['title'])) {
                    $fields = $re_arr;
                    break;
                }
        ...
        $fields = extra_array(D('common/Models')->filterFields($fields));
    }
...
{
    return D('common/PublicConfig')->config($name, $value);
}