...
class QrAdmin extends WebBase
{
    ...
    public function lists()
    {
        ...
        // 获取用户标签信息
        $tagArr = M( 'auth_group' )->where( wp_where($map) )->column('title', 'id');
    ...
    function add()
    {
        ...
        if (IS_POST) {
            ...
            if ($id) {
                $save['qr_code'] = D('home/QrCode')->addQrCode(I('action_name'), 'QrAdmin', $id);
                $map['id'] = $id;
                if ($save['qr_code']) {
                ...
                return $this->success('添加二维码成功！', U('lists?model=' . $model['name'], $this->get_param));
            } else {
