...
class WeixinAddon extends Weixin
{
    ...
    function reply($dataArr, $keywordArr = [])
    {
        if ($dataArr['Content'] == 'subscribe') {
            ...
            $config = D('common/PublicConfig')->getConfig('weixin', 'weixin_wecome', get_pbid()); // 获取后台插件的配置参数
            ...
            $config = getAppConfig('scan'); // 获取后台插件的配置参数
    ...
    function scan($dataArr, $keywordArr = [], $config = [])
    {
        ...
        if ($qr['addon'] == 'UserCenter') { // 设置用户分组
            ...
            if (!empty($qr_admin['tag_ids'])) {
                D('common/UserTag')->addTags($uid, $qr_admin['tag_ids']);
            }
            ...
            // 门店二维码
            // 触发会员卡图文
            $config = getAppConfig('Card'); // 获取后台插件的配置参数
            $articles[0] = array(
                'Title' => '点击进入免费领取微会员哦~',
            ...
            $pbid = get_pbid();
            $config = getAppConfig('Card', $pbid); // 获取后台插件的配置参数
            $articles[0] = array(
                'Title' => '点击进入免费领取微会员哦~',
                'Description' => $config['title'],
                'PicUrl' => SITE_URL . "/card/cover_pic.png",
             ...
             * M( 'public_follow' )->where ( wp_where( $map1 ) )->update([ 'uid'=> $qr ['extra_int'] ]);
             * M( 'user' )->where ( wp_where( $map2 ) )->delete ();
             * M( 'credit_data' )->where ( wp_where( $map2 ) )->delete ();
             */
            M('user')->where(wp_where(array(
                'uid' => $qr['aim_id']
            )))->update(['bind_openid'=>$dataArr['FromUserName']]);
