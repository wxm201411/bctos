...
class CustomMenu extends WebBase
{
    ...
    function custom_lists()
    {
        // 获取规则列表
        $map['pbid'] = get_pbid();
        $rules = find_data( M('custom_menu_rule')->where(wp_where($map))->select() );
        // dump($rules);
        $rule_lists = [];
        if (! empty($rules)) {
    ...
    private function _deal_data($d, $pid = 0)
    {
        ...
        if ($len > $max_len) {
            return $this->error($res['name'] . '菜单已超过' . $max_len . '个字的限制');
        }
        ...
        switch ($d['from']) {
            ...
            case 2: // URL11
                ...
                if ($len > 1024) {
                    return $this->error($res['name'] . ' 的URL已超过1024个字节的限制：' . $res['url']);
                }
                break;
            case 3: // 自定义
                ...
                if ($len > 128) {
                    return $this->error($res['name'] . ' 的关键词已超过128个字节的限制：' . $res['key']);
                }
                break;
            case 4: // 小程序
    ...
    public function send_menu()
    {
        ...
        if (! isset($res['errcode']) || $res['errcode'] == 0) {
            if ($rule_id > 0) {
                M('custom_menu_rule')->where('id', $rule_id)->update(['menuid'=>$res['menuid']]);
            }
            return $this->success('发送菜单成功');
        } else {
    ...
    private function check_count($data, $count)
    {
        if ($data['pid'] == 0) { // 最多 3 个一级菜单
            if ($count >= 3) {
                return $this->error('最多 3 个一级菜单');
            }
        } else { // 最多 5 个二级菜单
            if ($count >= 5) {
                return $this->error('该一级菜单下最多 5 个二级菜单');
            }
        }
    }
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            if ($len > $max_len) {
                return $this->error('菜单名已超过' . $max_len . '个字的限制');
            }
            
            if ($data['from'] == 2 && empty($data['url'])) {
                return $this->error('请先填写URL地址');
            }
            if ($data['from'] == 1 && (empty($data['material']) || $data['material'] == 'text:')) {
                return $this->error('请先选择素材');
            }
            ...
            if ($res !== false) {
                ...
                if ($data['pid'] > 0) {
                    ...
                    if ($from != 0) {
                        M('custom_menu')->where(wp_where($pmap))->update(['from'=>0]);
                    }
                }
                ...
                return $this->success('保存菜单成功！', $url);
            } else {
                return $this->error('保存菜单失败');
            }
        } else {
            // 获取一级菜单
            $map['pbid'] = get_pbid();
            ...
            $this->assign('page_tips', '可创建最多 3 个一级菜单，每个一级菜单下可创建最多 5 个二级菜单。编辑中的菜单不会马上被用户看到，请放心调试');
