...
class Publics extends WebBase
{
    ...
    public function step_0()
    {
        ...
        if (config('app.PUBLIC_BIND') && is_install('PublicBind')) {
        ...
        if (IS_POST) {
            ...
            if (empty($id)) {
                ...
                if ($id) {
                    D('common/User')->where('uid', $uid)->update(['wpid' => $id]);
                    // 更新缓存
                    ...
                    return $this->success('添加基本信息成功！', $url);
                } else {
                ...
                if ($res !== false) {
                    return $this->success('保存基本信息成功！', $url);
                } elseif ($res === 0) {
    ...
    public function step_2()
    {
        ...
        if (IS_POST) {
            ...
            $data = $this->checkData($data, $model);
            $res = $Model->where('id', $id)->save($data); // dump($res);
            if ($res !== false) {
                D('common/Publics')->clearCache($data['id']);

                return $this->success('保存成功！', U('weixin/publics/lists'));
            } else {
                return $this->error($Model->getError());
            }
        } else {
    ...
    public function waitAudit()
    {
        ...
        if ($is_audit == 0 && !config('app.REG_AUDIT')) {
    ...
    public function check_res()
    {
        ...
        // 获取微信权限节点
        $list = find_data(M('public_auth')->select());
        foreach ($list as &$vo) {
    ...
    function payment_set()
    {
        ...
        if (IS_POST) {
            ...
            if (empty($save['appid'])) {
                return $this->error('110101:APPID不能为空');
            }
            if (empty($save['mch_id'])) {
                return $this->error('110102:微信支付商户号不能为空');
            }
            if (empty($save['partner_key'])) {
                return $this->error('110103:API密钥不能为空');
            }

            if (!empty($data['appid']) && $save['appid'] != $data['appid']) {
                return $this->error('110104:appid与当前账号的appid不一致');
            }
            ...
            return $this->success('保存成功！', U('payment_set'));
        } else {
            $data['type'] = intval($data['type']);
            $this->assign('info', $data);

            $page_tips = '除了配置下面的参数，还需要在微信商户平台配置授权域名，<a href="' . U('payment_help') . '">查看配置教程</a>';
    ...
    function upload_wxfile()
    {
        if (IS_POST) {
            ...
            if (empty($data['wpid'])) {
                return $this->error('找不到公众号id');
            }
            try {
                $file = get_file_info($data['file_id']);
                copy(SITE_PATH . '/public/storage' . $file['savepath'] . $file['savename'], SITE_PATH . '/public/' . $file['name']);
            } catch (\Exception $e) {
    ...
    function check_url()
    {
        ...
        if (!Env::get('APP_DEBUG')) {
            if ($info['scheme'] == 'http') {
                return $this->error('110500:小程序需要在https环境下配置');
            }
            if ($info['host'] == 'localhost' || $info['host'] == '127.0.0.1') {
                return $this->error('110501:小程序需要有域名的环境下配置');
            }
        }
...
//         $map['id'] = $id = get_pbid();
        ...
        if (IS_POST) {
            ...
            if (empty($post['public_name'])) {
                return $this->error('请先填写小程序名称再提交');
            }
            if (empty($post['public_id'])) {
                return $this->error('请先填写原始ID再提交');
            }
