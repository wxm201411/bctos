...
//                     D('common/Custom')->replyNews($uid, $id);
        ...
        if (!isset($addons[$key])) {
            ...
            if ($keyword_cache === false || Env::get('APP_DEBUG')) {
                $keyword_cache = [];
                foreach ($addon_list as $k => $vo) {
                    // $keyword_cache[$vo['name']] = $k;
                    // $keyword_cache[$vo['title']] = $k;

                    $path = SITE_PATH.'/app/'. $vo['name'] . '/keyword.php';
                    if (file_exists($path)) {
        ...
        // dump($keywordArr);
        // 最终也无法定位到插件，终止操作
        if (!isset($addons[$key]) || !file_exists(SITE_PATH.'/app/'. $addons[$key] . '/model/WeixinAddon.php')) {
            echo 'success';
            exit();
        }

        // 加载相应的插件来处理并反馈信息
        require_once SITE_PATH.'/app/'. $addons[$key] . '/model/WeixinAddon.php';
