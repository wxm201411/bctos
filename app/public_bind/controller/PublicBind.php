...
class PublicBind extends WebBase
{
    ...
    {
        require_once(SITE_PATH.'/vendor/WXBiz/wxBizMsgCrypt.php');
        // 第三方发送消息给公众平台
        $encodingAesKey = config('app.ENCODING_AES_KEY');
        $token = SYSTEM_TOKEN;
        $appId = config('app.COMPONENT_APPID');
        ...
        if ($errCode == 0) {
            ...
            if (isset($data['ComponentVerifyTicket'])) {
                ...
                if (isset($publics['id'])) {
                    D('common/User')->where('wpid', $publics['id'])->update(['wpid'=>0]);
    ...
    public function after_auth()
    {
        ...
        $data['is_bind'] = 1;
        $data['encodingaeskey'] = config('app.ENCODING_AES_KEY');
        $data['cTime'] = NOW_TIME;
        ...
        if ($old_uid > 0 && $uid != $old_uid) {
            // 切换了用户
            D('common/User')->where('uid', $old_uid)->update(['wpid'=>0]);
            D('common/User')->clearCache($old_uid);
        }
        
        D('common/User')->where('uid', $uid)->update(['wpid'=>$pbid]);
