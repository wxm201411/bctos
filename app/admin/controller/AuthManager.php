...
class AuthManager extends Admin {
	...
	public function updateRules() {
		...
		$rules = find_data( M( 'AuthRule' )->where ( wp_where( $map ) )->whereIn('type','1,2')->order ( 'name' )->select() );
	...
	public function wechat() {
		...
		$this->meta_title = '用户组管理';
		$wx_type = find_data( M( 'auth_group' )->where('type',2)->select() );
		//dump($wx_type);
		$this->assign('wx_type',$wx_type);
		$list = find_data( M( 'auth_rule' )->where('type','public_interface')->order('id asc')->select() );
		//dump($list);
		$this->assign ( 'list_data', $list );
		return $this->fetch();
	}
	function set_switch() {
		...
		if ($tid && $rid && $val!==''){
		    ...
		    $rules = M( 'auth_group' )->where( wp_where($map) )->value('rule');
		    $rules = explode(',', $rules);
		    if($val){//添加
		    ...
		    $rules = implode(',', $rules);
		    $res = M( 'auth_group' )->update(['rule'=>$rules]);
	...
	public function access() {
		...
		$map2 ['status'] = 1;
		$rules = find_data( M( 'AuthRule' )->where ( wp_where( $map2 ) )->field ( true )->select() );
		foreach ( $rules as $vo ) {
	...
	public function writeGroup() {
		...
		if ($data) {
			if (empty ( $data ['id'] )) {
				...
				$r = $AuthGroup->where('id', $data['id'])->save($data);
			}
			if ($r === false) {
	...
	public function user($group_id) {
		...
		$auth_group = find_data( M( 'AuthGroup' )->field ( 'id,title,rules' )->select() );
		$prefix = DB_PREFIX;
