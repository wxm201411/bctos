...
//	);
	...
	public function getList($addon_dir = '', $type = 0) {
		...
		$list = find_data( $this->where ( wp_where( $where ) )->field ( true )->select() );
	...
	public function getAdminList() {
		$admin = [];
		$db_addons = find_data( $this->where("status=1 AND has_adminlist=1" )->field ( 'title,name' )->select() );
		if ($db_addons) {
			foreach ( $db_addons as $value ) {
				$admin [] = array (
						'title' => $value ['title'],
						'url' => "app/adminList?name={$value['name']}"
				);
			}
		}
		return $admin;
	...
	function set_show($id, $val) {
		$map ['id'] = $id;
		return $this->where ( wp_where( $map ) )->update([ 'is_show'=> $val ]);
	}
}
