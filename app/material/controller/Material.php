...
class Material extends \app\home\controller\Home
{
    ...
    public function initialize()
    {
        ...
        $res ['title'] = '文件素材';
        $res ['url'] = U('file_lists', $param);
        $res ['class'] = strpos($act, 'file') !== false ? 'current' : '';
        //$nav [] = $res;
    ...
    public function doAdd()
    {
        ...
        foreach ($data as $key => $vo) {
            ...
            if (empty($save['title'])) {
                return $this->error('请填写第' . $textArr[$key + 1] . '篇文章的标题');
            }
            if (empty($save['cover_id'])) {
                return $this->error('请上传第' . $textArr[$key + 1] . '篇文章的封面图片');
            }
            if (!empty($save['id'])) {
                // 更新数据
                $map2['id'] = $save['id'];
                ...
                if ($id) {
                    ...
                    return $this->error('增加第' . $textArr[$key + 1] . '篇文章失败，请检查数据后重试');
                }
            }
        }
        if (!empty($ids)) {
            ...
            M('material_news')->where(wp_where($map))->update(['group_id' => $group_id]);
        }
        // 如果有权限，则静默同步到微信，（用户自己手动上传）
//         $this->_syc_news($group_id, true);

        return $this->success('操作成功', U('material_lists'));
    }
...
//         $field = 'any_value(id) id,any_value(title) title,any_value(cover_id) cover_id,any_value(intro) intro,group_id,any_value(cTime) cTime';
        $data = M('material_news')->where(wp_where($map))
            ...
            ->paginate($row, $count);
        $list = dealPage($data);
        $field = 'id,title,cover_id,intro,group_id,cTime,forbit_del';
//         dump($list);
        $idsArr = getSubByKey($list['list_data'], 'id');
        $map1 = [];
        if (!empty($idsArr)) {
            $map1['id|group_id'] = array('in', $idsArr);
        } else {
            $map1['id'] = 0;
        }
        $all = M('material_news')->order('id desc')->where(wp_where($map1))->column($field, 'id');
        $groupArr = [];
        foreach ($all as $a) {
            $groupArr[$a['group_id']][] = $a;
        }
        foreach ($list['list_data'] as &$vo) {
            if (empty($vo['group_id'])) {
                $vo ['group_id'] = $vo ['id'];
                M('material_news')->where('id', $vo['id'])->update(['group_id' => $vo ['id']]);
            }
            if ($vo['count'] == 1) { //单图文
                if (isset($all[$vo['id']])) {
                ...
                $child_list = $groupArr[$vo['group_id']];
...
//         $field = 'any_value(id) id,any_value(title) title,any_value(cover_id) cover_id,any_value(intro) intro,group_id,any_value(cTime) cTime';
    ...
    {
        if ($group_id == '') {
            $group_id = $only_body = I('group_id');
        }
        $articles = D('Material')->get_news_by_group_id($group_id);
    ...
    public function _syc_news($group_id, $show_error = true)
    {
        ...
        if (!empty($media_id) && !$need_del) {
            ...
            if (isset($res['errcode']) && $res['errcode'] != 0) {
                ...
                M('material_news')->where(wp_where($map3))->update(['media_id' => $res['media_id']]);
                $newsUrl = $this->_news_url($res['media_id']);
                foreach ($news as $a) {
                    $map4['group_id'] = $group_id;
                    $map4['title'] = $a['title'];
                    M('material_news')->where(wp_where($map4))->update(['url' => $newsUrl[$a['title']]]);
                }
            }
        }
        return true;
    ...
    public function syc_news_from_wechat()
    {
        ...
        $has = M('material_news')->where(wp_where($map))->column('group_id','media_id');
        foreach ($list['item'] as $item) {
            ...
            if (isset($has[$media_id])) {
                ...
                if (!empty($ids)) {
                    ...
                    M('material_news')->where(wp_where($map2))->update(['group_id' => $ids[0]]);
                }
            }
        }
    ...
    public function picture_lists()
    {
        ...
        // dump($config);
        $this->assign('page_tips', '温馨提示：图片大小不超过2M,支持PNG\JPEG\JPG\GIF格式');
        $map['is_use'] = 1;
    ...
    public function add_picture()
    {
        ...
        if (empty($save['cover_id']) || empty($save['cover_url'])) {
            return $this->error('图片参数出错');
        }
        if (substr($save['cover_url'], 0, 9) == '/storage/') {
    ...
    {
        // $this->assign ( 'page_tips', '温馨提示：图片大小不超过5M, 格式: bmp, png, jpeg, jpg, gif' );
        $map['is_use'] = 1;
        $map['pbid'] = get_pbid();
        $page_data = M('material_image')->where(wp_where($map))
            ->field('id,cover_url')
    ...
    {
        if (I('img_id')) {
            $id = $only_body = I('img_id');
        }

        $imgpath = D('Material')->ajax_picture_by_id($id);
        if (isset($only_body) && $only_body) {
            echo $imgpath;
        } else {
            return $imgpath;
    ...
    public function syc_image_from_wechat()
    {
        ...
        $has = M('material_image')->where(wp_where($map))->column('id','media_id');
...
//                 addWeixinLog($data,'syc_image_from_wechat'.get_pbid().'--4444');
    ...
    {
        if ($id == '') {
            $id = $only_body = I('voice_id');
        }
        $voiceMaterial = D('Material')->ajax_voice_by_id($id);
        if (isset($only_body) && $only_body) {
    ...
    {
        $this->assign('page_tips', '温馨提示：语音大小不超过2M，长度不超过60秒，支持AMR\MP3格式');
        ...
        Config::load(SITE_PATH . '/app/home/config.php');
        ...
        if (!empty($list['list_data'])) {
            ...
            $file_list = find_data(M('file')->where(wp_where($file_map))->select());
            $cpath = config('app.download_upload');
            $cpath['rootPath'] = './storage/download';
    ...
    {
        // $this->assign ( 'page_tips', '温馨提示：语音大小不超过5M，长度不超过60秒，支持mp3/wma/wav/amr格式' );
        $map['is_use'] = 1;
    ...
    public function voice_add()
    {
        ...
        if (IS_POST) {
            ...
            if ($id) {
                return $this->success('添加' . $model['title'] . '成功！', U('voice_lists?model=' . $model['name'], $this->get_param));
            } else {
    ...
    public function voice_edit()
    {
        ...
        if (IS_POST) {
            ...
            if ($res !== false) {
                return $this->success('保存' . $model['title'] . '成功！', U('voice_lists?model=' . $model['name'], $this->get_param));
            } else {
    ...
    {
        $this->assign('page_tips', '温馨提示：视频不能超过10MB，支持MP4格式');
        $map['is_use'] = 1;
    ...
    {
        if ($id == '') {
            $id = $only_body = I('video_id');
        }
        $videoMaterial = D('Material')->ajax_video_by_id($id);
        if (isset($only_body) && $only_body) {
    ...
    {
        // $this->assign ( 'page_tips', '温馨提示：视频不能超过20M，支持大部分主流视频格式，超过20M的视频可至腾讯视频上传后添加' );
        $map['is_use'] = 1;
    ...
    public function video_add()
    {
        ...
        if (IS_POST) {
            ...
            if ($id) {
                return $this->success('添加' . $model['title'] . '成功！', U('video_lists?model=' . $model['name'], $this->get_param));
            } else {
                return $this->error('添加失败！');
            }
        } else {
    ...
    public function video_edit()
    {
        ...
        if (IS_POST) {
            ...
            if ($res !== false) {
                return $this->success('保存' . $model['title'] . '成功！', U('video_lists?model=' . $model['name'], $this->get_param));
            } else {
    ...
    public function syc_file_from_wechat()
    {
        ...
        $has = M('material_file')->where(wp_where($map))->column('id','media_id');
        foreach ($list['item'] as $item) {
            $media_id = $item['media_id'];
            if (isset($has[$media_id])) {
                continue;
            ...
            isset($data['wechat_url']) || $data['wechat_url'] = isset($item['url']) ? $item['url'] : '';
            $data['media_id'] = $media_id;
    ...
    public function _get_file_media_id($file_id, $type = 'voice', $title = '', $introduction = '')
    {
        ...
        if (!$fileInfo) {
            ...
            return $this->error('素材不存在：' . $file_id);
        }
        $path = SITE_PATH . '/public/storage/download/' . $fileInfo['savepath'] . $fileInfo['savename'];
        if (!file_exists($path)) {
            addWeixinLog('素材文件不存在：' . $file_id, '_get_file_media_id');
            return $this->error('素材文件不存在：' . $file_id);
        }
        ...
        if (!$res) {
            ...
            return $this->error('素材文件上传失败' . $file_id);
        }
        if (isset($res['errcode']) && $res['errcode'] != 0) {
            addWeixinLog(error_msg($res, '视频/语音素材上传'), '_get_file_media_id');
            return $this->error(error_msg($res, '视频/语音素材上传'));
        }
        if (isset($res['code']) && $res['msg'] != 0) {
            addWeixinLog($res['msg'], '_thumb_media_id');
    ...
    public function text_add()
    {
        ...
        if (IS_POST) {
            ...
            $id = $Model->removeOption('data')->insertGetId($data);
            if ($id) {
                return $this->success('添加' . $model['title'] . '成功！', U('text_lists?model=' . $model['name']));
            } else {
                return $this->error($Model->getError());
            }
        } else {
    ...
    public function text_edit()
    {
        ...
        if (IS_POST) {
            ...
            if ($res !== false) {
                return $this->success('保存' . $model['title'] . '成功！', U('text_lists?model=' . $model['name']));
            } else {
                return $this->error($Model->getError());
            }
        } else {
    ...
    public function check_file_size($fileId, $limSize, $strExt = 'mp3,wma,wav,amr', $checkExt = 1)
    {
        ...
        if ($size > $bs) {
            return $this->error('上传文件不能超过' . $limSize . 'M');
            exit();
        }
        if ($checkExt) {
            $ext = $files['ext'];
            ...
            if (!in_array($ext, $extArr)) {
                return $this->error('上传文件类型不支持，请上传扩展名为' . $strExt . '的文件！');
            }
        }
    }
    ...
    public function check_video_size($fileId, $limSize, $strExt = 'mp4,fl,f4v,webm, m4v,mov,3gp,3g2 ,rm,rmvb, wmv,avi,asf,mpg,mpeg,mpe,ts,div,dv,divx,vob,dat,mkv,swf,lavf,cpk,dirac,ram,qt,fli,flc,mod', $checkExt = 2)
    {
        ...
        if ($checkExt) {
            ...
            if (!in_array($ext, $extArr)) {
                // return $this->error ( '上传文件类型不支持，请上传扩展名为' . $strExt . '的文件！' );
                return $this->error('上传文件不支持' . $ext . '类型');
            }
        }
    }
    ...
    function file_lists()
    {
        $this->assign('page_tips', '温馨提示：文件不能超过' . ini_get('upload_max_filesize') . '，支持apk,zip,rar,doc,docx,txt,xls,xlsx,pdf,ppt,pptx格式');
        $map ['is_use'] = 1;
        ...
        $this->assign('add_url', U('file_add'));
        $this->assign($list);
    ...
    function file_add()
    {
        ...
        if (IS_POST) {
            ...
            $id = M('material_file')->insertGetId($data);
            if ($id) {
                return $this->success('添加' . $model['title'] . '成功！', U('file_lists'));
            } else {
                return $this->error('添加失败');
            }
            ...
            $fields['file_id']['validate_file_exts'] = 'apk,zip,rar,doc,docx,txt,xls,xlsx,pdf,ppt,pptx';
            $fields['file_id']['validate_file_size'] = intval(ini_get('upload_max_filesize')) * 1048576;//单位M 转为字节，1024*1024=1048576

            $this->assign('fields', $fields);
            $this->assign('post_url', U('file_add'));
            return $this->fetch('add');
        }
    }
    ...
    public function file_edit()
    {
        ...
        if (IS_POST) {
            ...
            if ($res !== false) {
                return $this->success('保存' . $model['title'] . '成功！', U('file_lists'));
            } else {
                return $this->error('保存失败');
            }
        } else {
            ...
            $this->assign('post_url', U('file_edit', $this->get_param));
