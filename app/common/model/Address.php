<?php

namespace app\common\model;

use app\common\model\Base;
/**
 * Shop模型
 */
class Address extends Base {
	protected $table = DB_PREFIX. 'shop_address';
	function getInfo($id, $update = false, $data = []) {
		$key = 'Address_getInfo_' . $id;
		$info = S ( $key );
		if ($info === false || $update) {
			$info = empty ( $data ) ? $this->find ( $id ) : $data;
			if (! isset( $info ['city_name'] ) && ! empty ( $info ['city'] )) {
				$info ['city_name'] = $this->getCityName ( $info ['city'] );
			}
			S ( $key, $info );
		}
		
		return $info;
	}
	function getCityName($city) {
		$ids = array_filter ( explode ( ',', $city ) );
		$map ['id'] = array (
				'in',
				$ids 
		);
		$list = M ( 'common_category' )->where(wp_where( $map ))->select ();
		foreach ( $list as $v ) {
			$arr [$v ['id']] = $v ['title'];
		}
		foreach ( $ids as $id ) {
			isset( $arr [$id] ) && $title [] = $arr [$id];
		}
		return implode ( ' ', $title );
	}
	function deal($data) {
		if ($data ['is_use'] == 1) {
			$this->where(wp_where( 'uid=' . $data ['uid'] ))->update([ 'is_use'=> 0 ]);
		}
		
		if (! empty ( $data ['id'] )) {
			$map ['id'] = $data ['id'];
			$res = $this->where(wp_where( $map ))->update( $data );
		} else {
			$res = $this->insertGetId( $data );
		}
		
		// 更新用户缓存
		$list = $this->where(wp_where( 'uid=' . $data ['uid'] ))->select ();
		foreach ( $list as $v ) {
			$this->getInfo ( $v ['id'], true, $v );
		}
		
		return $res;
	}
	function getUserList($uid) {
		$map ['uid'] = $uid;
		$list = $this->where(wp_where( $map ))->field ( 'id' )->select ();
		foreach ( $list as &$v ) {
			$v = $this->getInfo ( $v ['id'] );
		}
		
		return $list;
	}
	function getMyAddress($uid) {
		$map ['uid'] = $uid;
		$id = $this->where(wp_where( $map ))->order ( 'is_use desc, id desc' )->value( 'id' );
		
		return empty ( $id ) ? '' : $this->getInfo ( $id );
	}
}
