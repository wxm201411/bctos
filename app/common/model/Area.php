...
class Area extends Base {
	...
	protected $table = DB_PREFIX. 'area';
	...
	public function getAreaList($pid = -1, array $where = [])
	{
		...
		return $this->where(wp_where($where))->order('area_id ASC')->select();

		// $map = [];
		// $pid != -1 && $map['pid'] = $pid;
		// $data = $this->where(wp_where($map))->order('area_id ASC')->select(); 
		// return $data;
	}
	...
	public function getAreaById($id) {
		...
		if(!empty($id)) {
			$map['area_id'] = $id;
			$result = find_data($this->where(wp_where($map))->find());
		}
	...
	private function _MakeTree($pid, $level = '0') {
		$result = $this->where(wp_where('pid='.$pid))->select();
		if($result) {
