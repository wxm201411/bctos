...
class News extends base {
	...
	public function install() {
		$install_sql = SITE_PATH.'/app/news/install.sql';
	...
	public function uninstall() {
		$uninstall_sql = SITE_PATH.'/app/news/uninstall.sql';
	...
	public function news($data) {
		if (isset ( $data ['value'] )) {
			...
			$list = find_data( M( 'material_news' )->where ( wp_where( $map ) )->field ( $field )->order ( 'id asc' )->select() );
			$count = count ( $list );
			$main = $list [0];
			if ($count > 1) {
				unset ( $list [0] );
