...
class WapBase extends Base
{
    ...
    public function _empty($name)
    {
        ...
        if (!input('?return_html') && (IS_AJAX || input('?is_ajax'))) {
            ...
            if (isset($data['code']) && !isset($data['ex_code'])) {
                if ($data['code'] == 0) {
                    return $this->error($data['msg'], null);
                } else {
                    $url = isset($data['url']) ? $data['url'] : null;
                    return $this->success($data['msg'], $url);
                }
            }
            $this->assign($data);
    ...
    private function initPublic()
    {
        ...
        if (IS_GET) {
            ...
            // 设置版权信息
            $this->assign('system_copy_right', empty($manager['copy_right']) ? config('app.COPYRIGHT') : $manager['copy_right']);
            $tongji_code = empty($manager['tongji_code']) ? config('app.TONGJI_CODE') : $manager['tongji_code'];
    ...
    private function initUser($info)
    {
        ...
        if (!$uid) {
            ...
            $user['uid'] = $youke_uid;
            M('config')->where('name="FOLLOW_YOUKE_UID"')->update(['value' => $youke_uid]);
        ...
        //资源汇定制开发需求 TODO
        if (empty($myinfo['mobile']) && ACTION_NAME != 'login') {
            //header('location:' . U('ziyuanhui/wap/login', array('from' => 1)));
        }
