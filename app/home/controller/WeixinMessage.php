...
class WeixinMessage extends Home
{
    ...
    function person()
    {
        ...
        if ($creatTime[0] < $minTime) {
            $this->assign('page_tips', $toUser['nickname'] . ' 用户发送消息的时间超过48小时，管理员只可以给48小时以内发送消息的用户回复信息');
            $can_send = 0;
        } else {
            $can_send = 1;
        }
    ...
    function _deal_content($data)
    {
        ...
        require_once(SITE_PATH."/vendor/qqface.php");
    ...
    function reply()
    {
        ...
        switch ($msg_type) {
            ...
            case 'image':
                ...
                if (empty($mediaId)) {
                    return $this->error('该图片medial_id获取失败');
                }
                // 新增图片素材
                $param['msgtype'] = 'image';
        ...
        if ($msg_type == 'text') {
            return $this->success('回复成功', U('person', array(
