...
class AuthGroup extends Home
{
    ...
    public function initialize()
    {
        ...
        $this->qr_code = config('app.QRCODE');
    }
    ...
    public function lists()
    {
        ...
        if ($this->syc_wechat) {
            ...
            $page_tips = '温馨提示：当前用户组数据会与微信端的用户组实时同步，需要删除用户组请到微信后台删除。';
            // 搜索按钮
        ...
        $groupidArr = [];
        $list_data = $this->getModelList($this->model, 'id asc');
        ...
        if (!empty($groupidArr)) {
            ...
            // 获取关注用户的分组数量
            $uidsArr = find_data( M('auth_group_access')->where(wp_where($gmap))->select() );
        }
        ...
        if ($this->qr_code) {
            foreach ($list_data['list_data'] as &$vo) {
                ...
                $res = D('home/QrCode')->addQrCode('QR_LIMIT_SCENE', 'UserCenter', $vo['id']);
                if (!($res < 0)) {
                    $map2['id'] = $vo['id'];
                    M('auth_group')->where(wp_where($map2))->update(['qr_code' => $res]);
                    $vo['qr_code'] = $res;
                    $vo['qr_code'] = "<a target='_blank' href='{$vo['qr_code']}'><img src='{$vo['qr_code']}' class='list_img'></a>";
                }
            }
            $page_tips .= '当用户微信扫分组里的二维码时，用户会自动移到该分组中';
        } else {
            // 删除二维码一栏
    ...
    public function checkTitle($title, $id = 0)
    {
        $tLen = strlen($title);
        if ($tLen > 30) {
            return $this->error('分组名称不能超过30个字符，或10个汉字！');
        }
    ...
    public function edit($id = 0)
    {
        ...
        if (request()->isPost()) {
            ...
            if ($res !== false) {
                ...
                return $this->success('保存' . $model['title'] . '成功！', U('lists?model=' . $model['name']));
            } else {
                return $this->error($Model->getError());
            }
        } else {
...
//                     $param['group']['name'] = $v['title'];
    ...
    {
        $id = intval($_GET['id']);
        $res = D('home/QrCode')->addQrCode('QR_LIMIT_SCENE', 'UserCenter', $id);
    }
    ...
    public function export()
    {
        ...
        $data = find_data(M()::table($px . 'public_follow')
            ->alias('f')
...
//             '经验值',
    ...
    public function tongbu_follow()
    {
        ...
        $list = find_data( M('auth_group')->where(wp_where($map))->select() );
        foreach ($list as $v) {
        ...
        foreach ($follow_list as $follow) {
            ...
            M('public_follow')->where('id=' . $follow['id'])->update(['has_subscribe' => $has_subscribe]);
        }
        ...
        $url = U('tongbu_follow');
        return $this->success('同步用户数据中，请勿关闭', $url);
