...
class Update extends Home {
	...
	function follow_merge() {
		...
		$follow = find_data( M( 'follow' )->where ( wp_where( $map ) )->limit ( 10 )->order ( 'id asc' )->select() );
		if (! $follow) {
			die ( 'It is Over!' );
		}
	...
	function field_sort() {
		...
		foreach ( $list as $v ) {
			...
			// dump ( $field_sort );
			$map ['id'] = $v ['id'];
			$res = M( 'model' )->where ( wp_where( $map ) )->update([ 'field_sort'=> $field_sort ]);
			dump ( $res );
			lastsql ();
		}
		dump ( 'It is over' );
